...
type AppUpdateRuntimeState = unknown;
type AsrRealtimeSessionRequest = unknown;
type AsrRealtimeSessionResult = unknown;
type BrowserDiagnosticResult = unknown;
type BrowserRuntimeProfile = unknown;
type CoworkContextUsageFailureReason = unknown;
type CoworkContextUsageSource = unknown;
type CoworkGoal = unknown;
type CoworkMessageRailIndexItem = unknown;
type DataMigrationBackupResult = unknown;
type DataMigrationLastRestoreResponse = unknown;
type DataMigrationRestoreScheduleResult = unknown;
type HtmlShareAccessMode = unknown;
type HtmlShareConfigurableStatus = unknown;
type HtmlShareDisabledSource = unknown;
type HtmlShareSourceType = unknown;
type HtmlShareStatus = unknown;
type InstalledKitRecord = unknown;
type KitReference = unknown;
type KitSkillMetadata = unknown;
type ListLocalWebServicesOptions = unknown;
type LocalWebService = unknown;
type OpenClawGatewayRepairErrorCode = unknown;
type OpenClawSessionPatch = unknown;
type Platform = unknown;
type PresetAgent = unknown;
type ResolvedKitCapabilities = unknown;
type ShareDeploymentAnalyzeProjectInput = unknown;
type ShareDeploymentCreateNodeInput = unknown;
type ShareDeploymentDetectCandidatesInput = unknown;
type ShareDeploymentDetectCandidatesResult = unknown;
type ShareDeploymentGetByLocalServiceInput = unknown;
type ShareDeploymentProjectAnalysis = unknown;
type ShareDeploymentResult = unknown;
...
export interface ElectronBridge {
    ...
    scheduledTasks: {
        ...
        update: (id: string, input: Partial<unknown>) => Promise<{
            success: boolean;
            task?: unknown;
            error?: string;
        }>;
        ...
        listRuns: (taskId: string, limit?: number, offset?: number, filter?: unknown) => Promise<{
            success: boolean;
            runs?: unknown[];
            error?: string;
        }>;
        ...
        listAllRuns: (limit?: number, offset?: number, filter?: unknown) => Promise<{
            success: boolean;
            ready?: boolean;
            runs?: unknown[];
            error?: string;
        ...
        onRunUpdate: (callback: (data: unknown) => void) => () => void;
