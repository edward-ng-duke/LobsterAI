...
> 本文档是 SaaS 改造的**开发落地清单**：把仓库中现有的每一个 Electron IPC 通道（`ipcRenderer.invoke` / `ipcRenderer.on` / `webContents.send`）逐个映射到目标 Web 架构的 REST 端点或 WebSocket 频道，并标注 v1/后续/降级。适合读者：正在改造 `src/renderer/services/*` 浏览器桥、编写后端 API、或做前后端联调对齐的工程师。表格可直接作为逐通道的开发任务清单（tracker）。
>
...
| **目标接口** | 建议的 REST 路径或 WS 频道。REST 前缀统一 `/api/v1`；WS 统一走一条多路复用连接 `/ws`（按 `channel` 字段分发）。 |
| **HTTP 方法 / WS 类型** | REST 动词，或 `WS(server→client)` / `WS(client→server)`。 |
| **v1** | ✅ v1 必做；🕒 后续（IM/浏览器等）；⛔ 不做（computer-use/VM）；🔻 降级（桌面能力在 web 无对等）。 |

### 约定：不再逐个函数签名，统一走三类通道

现有 IPC 面（`grep` 实测：主进程约 **200+** 个 `ipcMain.handle/on` 注册，散落在 `src/main/main.ts`（约 10560 行、大量 handler）与 `src/main/ipcHandlers/*/handlers.ts` 模块；主进程约 **29** 个 `webContents.send` 事件名，另有参数化的 `api:stream:${requestId}:*`）。目标端统一收敛为三类：
...
- **流式/事件类**（原 `webContents.send` + `ipcRenderer.on`）→ 一条 WS 连接上的**频道帧**：`{ "channel": "cowork:stream:message", "data": {...} }`。前端桥把 `electron.on('cowork:stream:message', cb)` 翻译成"订阅该 channel"。
...
| IM 渠道 | `im:*` | `/api/v1/im/*`（后续） | 🕒 | v1 不做，见 `13`。 |
...
> 下列为 `R→M`（`invoke`）通道。REST 前缀省略 `/api/v1`。租户隔离由 JWT 承载，路径不显式带 `tenantId`（见 `05`、`14-安全合规与多租户隔离.md`）。

### A.2.1 Cowork 会话（核心，v1）
...
### A.2.2 Cowork 配置 / 记忆 / 引导 / dreaming（v1）
...
| IPC 通道 | 方向 | 目标接口 | 方法 | 说明 | v1 |
|----------|------|----------|------|------|----|
| `cowork:permission:respond` | R→M | `/permissions/{requestId}:respond` | POST | 回应权限请求（allow/deny/scope）。配套 WS 推送 `cowork:stream:permission`。见 `src/main/main.ts:6752`。 | ✅ |
...
IM 全域 v1 不做（见 `13`）；此处仅列骨架，映射细节留待 IM 阶段。三方模型账号 OAuth（GitHub Copilot / OpenAI Codex 的 device-code 代持）为 **v1.x 后续**：v1 仅提供"平台托管 key + 用户自带 BYOK API key"，不含 device-code 代持，口径与 `13`、`01`、`09` 一致。

| IPC 通道（代表） | 方向 | 目标接口 | 方法 | 说明 | v1 |
|------------------|------|----------|------|------|----|
...
一条 WS 连接（`/ws`，JWT 鉴权，见 `03`、`14`）多路复用所有推送。前端桥把每个 `electron.ipcRenderer.on(channel, cb)` 注册为对该 `channel` 的订阅，把 `webContents.send(channel, payload)` 还原为收到 `{channel,data}` 帧后回调。

```json
// server → client
{ "channel": "cowork:stream:message", "data": { "sessionId": "s1", "message": { } } }
// client → server（订阅/取消/控制）
{ "op": "subscribe",   "channel": "artifact:file:changed", "params": { "filePath": "..." } }
{ "op": "unsubscribe", "channel": "artifact:file:changed", "params": { "filePath": "..." } }
{ "op": "cancel",      "channel": "model:stream", "params": { "requestId": "r1" } }
```
...
sequenceDiagram
  ...
  BE-->>UI: WS {channel:"cowork:stream:message",data}
  BE-->>UI: WS {channel:"cowork:stream:messageUpdate",data}
  Note over UI,BE: 权限回环
  SB-->>BE: permissionRequest
  BE-->>UI: WS {channel:"cowork:stream:permission",data}
  UI->>BE: POST /permissions/{reqId}:respond
  BE->>SB: ws 权限应答
  SB-->>BE: complete
  BE-->>UI: WS {channel:"cowork:stream:complete",data}
```

### A.3.2 Cowork 流式（v1，最关键）

发送点见 `src/main/main.ts:2187-2305`。这些是产品体验核心，必须 v1 完整实现。

| IPC 事件（send） | 方向 | WS channel | 载荷要点 | v1 |
|------------------|------|-----------|----------|----|
...
| `cowork:stream:contextUsage` | M→R | 同名 | `{sessionId, usage}` | ✅ |
...
| IPC 事件（send） | 方向 | WS channel | 说明 | v1 |
|------------------|------|-----------|------|----|
...
- **枚举来源**：`ipcMain.handle/on`（`src/main/main.ts` + `src/main/ipcHandlers/*/handlers.ts`，部分通道用 `src/shared/*/constants.ts` 常量）；`webContents.send` 事件名共 29 个（`grep` 实测），另有参数化 `api:stream:${requestId}:*`。本附录已逐域覆盖全部代表通道及全部 send 事件。
