...
> **挂载单位 = 工作区（对齐 07 并锁定为附录 C D5）**：本文的「虚拟工作区」就是现状 OpenClaw 的 state-dir 单元 `workspace-{agentId}`（main 为 `workspace-main`），映射到每租户 PVC 的子路径。挂载 / 隔离 / 锁的粒度统一为**工作区**，`session : workspace` 为**多对一**（一个工作区可服务同租户多会话，见 §7.3）。此前三处口径打架（07 曾按会话 `sessions/{id}`、本文早期按 `ws-{id}`、源码按 `workspace-{agentId}`）已由附录 C D5 拍死为「按工作区」；本文出现的 `ws-{workspaceId}` 仅为叙述别名，物理落点与 07、源码一致，**不是第三套方案**。
>
...
| OpenClaw 动态 state：`openclaw.json`、gateway token、port、临时日志 | Pod state 卷 / Secret / stdout | 由 `07` config sync 和 K8s Secret 管理，不进用户可浏览工作区 |
| 用户项目文件、agent 产物、可见工作目录 | PVC `project/` + S3 镜像 | 本文档主责，向前端和 File Service 暴露逻辑路径 |
...
| state 隔离 P0 | 浏览器/API 尝试列出 `.openclaw-state`、token、config、日志 | 返回 404/403，不泄露存在性和内容 |
...
> ```
>
> 本文件服务用到的错误码及状态：`PAYLOAD_TOO_LARGE`（**413**，文本 / body 过大，提示改走 `as=url` 或分片上传）、`QUOTA_EXCEEDED`（**507**，超存储配额）、`FORBIDDEN_PATH`（**403**，路径越界 / symlink 逃逸）、`NOT_FOUND`（**404**，工作区 / 文件不存在或跨租户，不泄露存在性）。**注意 `QUOTA_EXCEEDED` 是 507 而非 413**（附录 C §3.4 把二者拆开）。
...
| 浏览目录 | `GET /api/v1/workspaces/:wid/tree?path=` | `dialog:selectDirectory`（浏览语义）| 返回目录项（名称/类型/大小/mtime）|
| 读文本 | `GET /api/v1/workspaces/:wid/files?path=&as=text` | `dialog:readTextFile` | UTF-8 文本，>1MB 拒绝走此路（改下载）|
| 读二进制/预览 | `GET /api/v1/workspaces/:wid/files?path=&as=url` | `dialog:readFileAsDataUrl` | 返回**对象存储签名 URL**，不返回 data URL |
| 文件元信息 | `HEAD /api/v1/workspaces/:wid/files?path=` | `dialog:statFile` | size/mtime/mime/etag 在响应头 |
| 小文件写入 | `PUT /api/v1/workspaces/:wid/files?path=` | `dialog:saveInlineFile` | body ≤ 5MB；大文件走直传 |
...
| 下载 | `GET /api/v1/workspaces/:wid/files:download?path=` | `shell:openPath`（下载语义）| 302 到对象存储签名 URL |
| 删除（软） | `DELETE /api/v1/workspaces/:wid/files?path=` | （新增）| 移入 `.trash/`，配额巡检时物理清理 |
| 缩略图 | `GET /api/v1/workspaces/:wid/thumbnail?path=` | `dialog:generateThumbnail` | 服务端生成 + 缓存，返回签名 URL |
...
```http
GET /api/v1/workspaces/ws_abc/tree?path=docs&depth=1&cursor=&limit=1000
```
...
```http
GET /api/v1/workspaces/ws_abc/files?path=docs/report.md&as=text
-> 200 { "path": "docs/report.md", "encoding": "utf-8", "content": "..." }

GET /api/v1/workspaces/ws_abc/files?path=assets/cover.png&as=url
-> 200 { "path": "assets/cover.png", "url": "https://s3.../...&X-Amz-Signature=...", "expiresIn": 300, "mime": "image/png" }
```

- `as=text`：仅当 mime 为文本且 size ≤ 阈值（如 1MB）；否则返回 `413 PAYLOAD_TOO_LARGE`（统一错误信封，`details` 带 `size` / `limit`），并提示改用 `as=url` 拉取——注意这条 413 是「文本过大」而非「超配额」，后者是 `507 QUOTA_EXCEEDED`（附录 C §3.4）。
...
- 配额在 `POST /uploads` 阶段对 **Redis 预留计数**做原子 `INCRBY` 预留（reserve），`landed` 时按真实大小结算回补，超时未 `complete` 则原子回补释放（见 §8.1）。
...
| OpenClaw 挂载点 | `/workspace` → `workspace-{agentId}/project`（本文 `ws-{id}` 别名）| agent 的 cwd = 工作区根（对齐桌面端 `~/lobsterai/project`）|
| 文件服务挂载点 | `/data/tenants/{T}` | 只挂载本租户子树；跨租户不共享卷 |
| 隔离粒度 | 每租户一个 PVC（GA 主线）| 同租户多工作区共用 PVC 下不同 `workspace-*` 子目录；跨租户物理隔离 |
...
3. **记忆/托管段落写入归属单一持锁 Pod（附录 C D5）**：`MEMORY.md`、dreaming 产物、`AGENTS.md` 的 LobsterAI 托管段落即使物理上落在同一工作区卷内，也**只由当前持有 §6.3 写者租约的那个 Pod 写入**；未持锁的会话 Pod 对这些文件只读，杜绝两个 Pod 并发写导致记忆交错/损坏。
4. **权限矩阵**：文件服务 SA、OpenClaw Pod SA 均无跨租户卷挂载权限；由 07 的 K8s RBAC + PVC 命名空间隔离保证。

### 6.3 单写者租约（对齐附录 C D5）

因为 `session : workspace` 是多对一，同租户可能有多条会话（多个 OpenClaw Pod）指向**同一工作区**。若放任它们以 RWX 并发写同一批文件与 `MEMORY.md`，会出现丢写/交错/损坏。D5 拍死的口径是**同一工作区同一时刻只允许一个 RW 挂载**。本节给承接要点（完整租约协议见「后续设计」，字段级以 `libs/shared/contracts` 为准）：

- **租约存储**：分布式租约 = Redis 键（`lease:ws:{workspaceId}`，带 TTL、持有者 `podId` 与 `fencing token`）+ 工作区卷内的 lease 文件（Redis 故障时的兜底与自证）。二者一致才授予 RW。
- **获取/续租/释放**：会话调度到工作区时先抢租约；持锁 Pod 周期续租，正常退出/被抢占时释放；崩溃则 TTL 到期后可被接管，靠 fencing token 防脑裂下旧持有者迟到写。
- **未获租约的行为**：其余会话请求**排队等待**或**降级只读**（可浏览、可 `as=url` 预览，写入/落地被拒，返回统一错误信封的冲突语义码，具体码型入契约）。
- **记忆写归属**：`MEMORY.md`、dreaming 产物、`AGENTS.md` 托管段落只在持锁 Pod 写入（见 §6.2 第 3 点）。
- **验收**：并发写同一文件/`MEMORY.md` 不丢写、不交错、不损坏（对应 §2.4「单写者租约 P0」行、§10.3 与 16 的并发写门）。
...
window.electron.dialog = {
  ...
  // 新: ≤5MB 走 PUT /files；>5MB(至 25MB) 桥内改走 §4.4 上传流程，再把结果映射回旧形状
...
- **上限差异**：现状 `saveInlineFile` 允许 25MB，而新 `PUT /files` body 阈值为 5MB（§4.4）；桥须在 5MB–25MB 区间**自动转走上传（直传/分片）流程**，对调用方仍表现为一次成功的 `saveInlineFile`，避免中断 `ArtifactPanel`/`CodeRenderer`。
...
| 上传预留 | `POST /uploads` 原子预留、`landed` 结算、超时原子释放 | Redis 预留计数（权威）|
...
- **写入前置检查**：`PUT /files` 与 `POST /uploads` 前对 Redis 计数原子预留 `当前用量 + 增量`，若 `> 配额` 则回滚预留并返回 `507 QUOTA_EXCEEDED`（统一错误信封，`details` 带 `used` / `quota`）。注意此处是 507，**不是** 413（413 专指文本/body 过大 `PAYLOAD_TOO_LARGE`，见 §4.3）。
...
- [ ] 超配额写入返回 `507 QUOTA_EXCEEDED`（非 413）；读取不受影响；Redis 预留计数与实际 `du` 的**漂移 ≤ 用量的 1%，且绝对值 ≤ 100MB**（超阈值触发对账纠偏并告警）。
