...
| `memberships` | 用户↔租户多对多 + 角色 | `role` owner/admin/member；`UNIQUE(user_id, tenant_id)` |
...
- **本节定位**：本文只承接归属与落地要点；完整字段/约束/加密与订阅生命周期不在此展开，权威在附录 C §4/§6（deferred，见文末交叉引用）。
...
- **决策基线（权威裁决层）**：跨文档决策 D1–D16（RLS 强制 D2、身份表 owner D3、消息游标 D10 等）、§2 源码订正表、**身份/租户表字段级 DDL §4**、RLS 参考实现 §5、**计费引擎 schema §6（deferred，本文未展开完整字段）**：[附录C-决策基线与接口契约总纲](./附录C-决策基线与接口契约总纲.md)
