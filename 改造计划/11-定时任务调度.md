...
model ScheduledTask {
  ...
  // 调度引擎游标
  bullmqJobKey  String?  @map("bullmq_job_key")   // repeatable 走 upsertJobScheduler，schedulerId=task.id；delayed 存 jobId=task.id:scheduledForMs
...
sequenceDiagram
    ...
    alt schedule.kind == cron / every
        API->>Q: queue.upsertJobScheduler(task.id, {pattern|every, tz}, data)
    else schedule.kind == at
        API->>Q: queue.add(name, data, { delay: at-now, jobId: task.id:scheduledForMs })
    end
    API->>PG: UPDATE bullmq_job_key
    API-->>UI: 201 ScheduledTask
```

> **BullMQ API 与版本钉死**：重复任务统一走 **Job Scheduler API `queue.upsertJobScheduler(schedulerId, repeatOpts, template)`**，不用已弃用的 `queue.add({ repeat })` / `removeRepeatableByKey`（老 API 的 repeat key 会因 pattern/tz 变化而漂移，导致「删不掉旧 job」）。`schedulerId` 固定取 `task.id`，同 key `upsert` 天然幂等。`bullmq` 版本锁定到含 Job Scheduler API 的线（`bullmq@^5.34`，`upsertJobScheduler` 已 GA），精确补丁号写进 `package.json` 并纳入 lockfile，避免 Worker/Service 两侧版本漂移导致队列语义不一致。

- **创建**：写 Postgres → 按 `schedule.kind` 注册：`cron`/`every` → `upsertJobScheduler(task.id, ...)`；`at` → `queue.add(name, data, { delay, jobId: task.id:scheduledForMs })`。schedulerId/jobId 均由 `task.id` 派生，保证同一任务不重复入队（按次入队去重见 §4.4）。
- **更新**：`cron`/`every` 直接 `upsertJobScheduler(task.id, 新 repeatOpts)` 幂等覆盖（无需先删旧 key）；`at` 先 `queue.remove(旧 jobId)` 再 add；`enabled=false` 时 `removeJobScheduler(task.id)` / 移除 delayed job，但保留 Postgres 定义。
- **删除**：Postgres 软删/硬删 + `removeJobScheduler(task.id)` / 移除 delayed job。
- **兜底扫描器**（应对超长 `at`、Redis 丢失恢复、scheduler 漏产出）：一个每分钟跑的 K8s CronJob 平台任务，**以 Postgres + `scheduled_task_runs` 对账为准**，而非依赖某个「未入队」布尔列（现状无该列，不可作判据）。逻辑：① 对 `enabled` 且 `schedule_kind='at' AND schedule_at <= now()+interval '2 min'` 的任务，若该 `(task_id, scheduledForMs)` 尚无 run 行、且队列中查不到对应 delayed job，则补 `queue.add` 入队；② 对 `cron`/`every` 任务，校验其 `upsertJobScheduler(task.id)` 仍存在，缺失则重新 upsert。入队后仍由 run 唯一约束 `@@unique(taskId, scheduledForMs)` 收口重复。这是「以 Postgres 为准、Redis 为加速」的对账机制，Redis 崩溃重建后能自愈。
...
sequenceDiagram
    ...
    W->>Lock: acquire lease(lease:task:{id}:{scheduledForMs}, TTL)
...
| 机制 | 实现 | 目的 |
|---|---|---|
| 任务级租约（lease+心跳） | Redis 租约 `lease:task:{id}:{scheduledForMs}`（带 TTL），Worker 抢占后周期性续租，并同步写 run 行 `lease_owner/lease_expires_at/heartbeat_at` | 同一任务同一触发时刻只跑一个 Worker；租约把「谁在跑、跑到几点过期」变成可判定状态 |
...
| 按次入队去重 | repeatable 由 `upsertJobScheduler(task.id)` 保证同 key 只有一个 scheduler；每次触发产出的 job 以 `(task.id, scheduledForMs)` 去重（delayed `jobId=task.id:scheduledForMs`），配合 run 唯一约束实现「每个触发时刻只入队/执行一次」 | 防止重复注册 scheduler + 防止同一触发时刻重复入队 |
...
「幂等」核心：Worker 消费是 **at-least-once**，靠「Redis 租约 + DB 唯一约束」把它收敛成 **effectively-once**。`scheduledForMs` 是「本次调度对应的计划时刻」，是幂等键（`@@unique(taskId, scheduledForMs)`）与租约 key 的核心——**注意它不能取 `job.opts.repeat`**：`job.opts.repeat` 是重复规格（pattern/every/tz），是配置不是时间戳。正确来源：`cron`/`every` 走 Job Scheduler，其产出的每个 job 携带当次迭代的计划毫秒（编码在产出 job 的 id/name 后缀里，Worker 解析后写入 job data 与 run 行）；`at` 直接取 delayed 的目标时刻。手动 `run` 触发取 `now`。
...
1. **定位 run**：`POST /scheduled-tasks/:id/stop` → Scheduler Service 查该任务最新 `status='running'` 的 run（取其 `scheduledForMs / sessionKey / lease_owner`）。若无进行中 run，返回 `CONFLICT`（见 §6 错误码）。
2. **下发取消信号**：向持有该 run 租约的 Worker（`lease_owner`）经 Redis pub/sub 命令通道下发 `cancel(task.id, scheduledForMs)`（与 §4.4 的多 Worker 反向寻址一致，禁止静默 no-op，语义对齐附录 C **D7**）。
...
`chat.abort`、`killSandbox`、`releaseSandbox('immediate')` 的具体契约以 [07](./07-OpenClaw运行时编排与沙箱隔离.md) 为权威，本节只定义定时任务侧的取消编排。**完整的取消命令 DTO / 命令通道 payload / 幂等（重复 stop）语义**记入待补设计，字段级以附录 C §3 契约事实源为准（见 deferred）。
...
| `scheduledTask:stop` | `POST /scheduled-tasks/:id/stop` | **全新能力**（现状 handler 空实现 `return{result:false}`、OpenClaw 无 stop API，见附录 C A7）：取消当前 turn（`chat.abort`）+ 回收/kill 沙箱，机制见 §4.7 |
...
| `scheduledTask:listChannels` / `listChannelConversations` | `GET /scheduled-tasks/channels`（GA 主线 IM 未接，返回空/仅站内） | IM 渠道，GA 后续 |
...
`ScheduledTaskInput`（`types.ts:92-103`）作为 REST body schema 基本沿用，仅去掉 IM 相关 `delivery` 分支（GA 主线）。响应体 `ScheduledTask`（含派生 `state`）保持字段兼容，前端组件（`ScheduledTasksView.tsx` / `TaskForm.tsx` / `TaskDetail.tsx`）几乎无需改结构，只改数据来源（REST/WS 替代 IPC）。
...
- **统一错误信封**（所有 REST 4xx/5xx，同附录 C §3）：`{ "error": { "code", "message", "requestId", "details" } }`。本域关键错误码：`NOT_FOUND`（任务/run 不存在）、`VALIDATION_FAILED`（schedule/cron 表达式或 tz 非法）、`QUOTA_EXCEEDED`（任务数/触发频率超配额，见 [09](./09-模型代理与计费.md)/[14](./14-安全合规与多租户隔离.md)）、`CONFLICT`（`stop` 目标非 running）、`PERMISSION_DENIED`（跨租户，见 [14](./14-安全合规与多租户隔离.md)）。
...
> 完整的字段级 DTO、错误码全量枚举与三事件 payload schema 落在 `libs/shared/contracts`，不在本文内联展开（见 deferred）。
