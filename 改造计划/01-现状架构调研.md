...
2. 因此改造成本集中：**把 `window.electron` 的实现从「IPC 桥」换成「HTTP+WS 桥」**，只要保持全局表面同接口，绝大多数组件与 slice 无需改动；但组件内直连 `window.electron` 的调用与 services 一样依赖该全局桥，不能遗漏（03 会给清单）。
3. 桥接层要同时覆盖两种语义：
...
- **事件适配**：`openclawRuntimeAdapter.ts` 把网关的 `message/messageUpdate/sessionStatus/contextUsageUpdate/contextMaintenance/permissionRequest/complete/error` 事件翻译为 §5.1 的 Cowork 流事件。`chat.send` 的应用层拦截线是安全线常量 `OPENCLAW_CHAT_SEND_PAYLOAD_SAFE_LIMIT_BYTES`；`30MB`（`30*1000*1000`，十进制）是 WS 帧硬限，超限触发 close 1009（见附录 C §2-B15）。
