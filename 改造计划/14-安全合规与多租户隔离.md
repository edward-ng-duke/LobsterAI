...
- **不做** IM 渠道（后续）——相关 connector 配置 v1 仅记录不激活（见 `10` §6、`13`）。
...
> 本节给**安全基线（做什么、为什么）**；Pod 编排、生命周期、预热池等**怎么做**见 `07-OpenClaw运行时编排与沙箱隔离.md`。以下 YAML 与 `07` §8 一致，此处作为安全权威重述。
...
| **gVisor（runsc）** | 强（用户态内核拦 syscall）| **v1 默认**基线 |
...
- **NetworkPolicy 默认拒绝**入站（仅放行后端 Cowork Service 连 gateway 端口）与横向（Pod 间默认不可互通），杜绝跨租户/跨会话横向移动（YAML 见 `07` §8.3）。这样即使 `gateway.bind=0.0.0.0`（Pod 内），网络层仍等价于原 loopback 语义。
...
> 令牌体系、Prisma 全局过滤、RLS 策略的**完整实现**在 `05-认证与多租户账户.md` §6；本节把它上升为**平台级不可违反的红线**，并统一"越权即 404"的口径。
...
3. **双层过滤，任一失效不导致越权。** 应用层（Prisma tenant extension 自动注入 `where tenant_id`）+ 数据库层（RLS `FORCE ROW LEVEL SECURITY`）。
...
| 缓存/Redis key 无租户前缀 | 所有 key 前缀 `t:{tid}:`；Pub/Sub 频道 `cowork:stream:{sessionId}` 按会话隔离 | PEN-ISO-5 |
| 对象存储路径无租户前缀 | S3 key 强制 `tenants/{tid}/...`，服务端拼接，**绝不接受客户端传完整 key** | PEN-ISO-6 |
| 签名 URL 越权 | 对象级、限时效、绑定 tenant/session（§5.4）| PEN-CONTENT-3/4 |
| OpenClaw 工作区跨租户复用 | 每租户/每会话独立 Pod + PVC（`07`）；预热 Pod 净室、claim 后只服务一个会话即销毁 | PEN-ISO-7 |
...
- 沙箱 Pod **直连外网被 NetworkPolicy 拒绝**（仅放行 egress-proxy 端口 + DNS），出网只此一路。
...
- 沙箱域**不设**任何主应用登录 cookie；后端不接受来自沙箱域、无 Bearer token 的凭据请求。
...
Content-Security-Policy:
  default-src 'none';
  script-src 'unsafe-inline' 'unsafe-eval' blob:;      # AI 产物常内联/eval，仅在沙箱域放开
...
- `script-src` 放开 `unsafe-inline`/`unsafe-eval` **仅限沙箱域**（因 AI 产物常内联脚本），**主应用域绝不放开**。
...
| 绑定 | 签名载荷含 `tenantId|sessionId|artifactId|path|exp`，HMAC-SHA256，密钥仅后端持有 |
| 时效 | 读 ≤ 300s（预览类 ≤ 10min）；上传 PUT ≤ 900s；过期即 403 |
...
| 密钥轮换 | 签名密钥定期轮换（§6.5）；旧密钥保留一个过期窗口以免打断在途 URL |
...
| 命令（MCP stdio）| 任意命令注入 | `commandSafety` 白名单，默认 `managed_npx_only`，拒任意二进制（§8、`10` §4.2）|
...
2. **风险分级门控**（复用 `SkillSecurityReport` 风险分：info=0/warning=5/danger=20/critical=50，`skillSecurityScanner.ts:53-58`）：
   - `safe/low` → 自动 `ready`；
   - `medium/high` → 需用户显式确认后 `ready`，否则 `blocked`；
   - `critical` → **默认 `blocked`**，仅企业租户管理员可强制放行且必记审计。
3. **命令白名单**：stdio MCP 默认 `managed_npx_only` + `requireScan=true`；**永不**默认允许任意本地二进制（§7、`10` §4.2）。
...
```mermaid
flowchart LR
  U[租户/用户请求导出] --> API[POST /v1/privacy/export]
  API --> Q[BullMQ 导出任务]
...
- **级联覆盖面**（缺一不可）：Postgres 业务表、对象存储全前缀（含分享/导出/备份）、PVC 卷子树、Vault 密钥、Redis 缓存/路由、审计中的**内容**（保留脱敏元数据）。
...
| PEN-ISO-5 | 检查 Redis/缓存/Pub-Sub key 前缀 | 均含 `t:{tid}:` / 按 session 隔离 |
...
| 编号 | 测试动作 | 通过判定 |
|---|---|---|
| PEN-SUP-1 | 安装含 data-exfiltration 的 skill（critical）| 默认 blocked，普通租户无法启用 |
| PEN-SUP-2 | 提交 `command=/bin/sh` 的 stdio MCP | 返回 422（commandSafety 拒），不落地 |
