...
| 跨租户串数据防护（跨租户/IDOR/需隐藏存在性=404、tenant 过滤、RLS）| ✅ §3 | 令牌/RLS 落地见 `05`，落表见 `06` |
...
| 平台密钥 | 上游模型/ASR API key、S3/DB 凭据、JWT 签名私钥、云凭证 | 平台级沦陷、成本失控 |
...
| 跨租户越权（IDOR/横向）| 用 A 的令牌读 B 的会话/文件/artifact | JWT tid + RLS + 跨租户/IDOR 返回 404（§3）|
...
- 每个沙箱 Pod 的 spec 必须显式写 `runtimeClassName`：默认 `gvisor`，由编排器按 `resourceClass`/租户策略在 Pod 级注入；`kata` 用于高合规/企业档以及 V1 兼容矩阵判定的 gVisor 不兼容工作负载（对齐附录 C D4），不得把 RuntimeClass 做成只能整租户/整 namespace 切换的口头开关。
...
securityContext:
  ...
  readOnlyRootFilesystem: true          # 根只读；可写仅 /workspace(project + state 工作区) /state(运行时动态 state) /tmp(emptyDir sizeLimit)
  capabilities:
    drop: ["ALL"]
  seccompProfile:
...
- OpenClaw Pod 挂载同一目标工作区子路径（`tenants/{tenantId}/ws-{workspaceId}`；`workspace-{agentId}` / `workspace-main` 仅为桌面现状来源名，见附录 C D5），但普通 agent 工具进程的 cwd 限在 `/workspace/project`；`/workspace/state`（`MEMORY.md`/`SOUL.md` 等）、`project/.trash/`、`quota.json` 等控制目录只能由 Config Sync/runtime/file service 受控路径访问，普通文件 API 不暴露（见 `08` §6.2）。
- **符号链接逃逸防护**：普通文件读写前对最终目标 `fs.realpath` 校验仍在 `/workspace/project` 根内；对 `/workspace/state` 的 Config Sync/runtime 写入走独立受控入口与工作区写者租约；挂载启用 `nosymfollow`（由 `07` Pod 安全策略保证）。
...
- **NetworkPolicy 默认拒绝**入站（仅放行后端 Cowork Service 连 gateway 端口）与横向（Pod 间默认不可互通），杜绝跨租户/跨会话横向移动（YAML 见 `07` §8.3）。容器入口通过 `--bind lan/auto` 解析为 Pod 内 `0.0.0.0`，但网络层仍等价于原 loopback 语义；不要把 `0.0.0.0` 写成 `openclaw.json` 字段或裸 `--bind` 参数（见 `15` §3.2）。**egress 亦默认拒绝，但须显式放行四类内网回调 + kube-dns**（见 §4.2 / 附录 C D8）。
- **每租户 namespace + ResourceQuota / LimitRange**：把租户的 CPU/内存/Pod 数封顶在其配额内。
- **Orchestrator 是唯一持 K8s 写权限的服务**，RBAC 最小化（ClusterRole 只含所需动词，RoleBinding 模板只绑定到受管 `sandbox-*` 租户 namespace），租约 TTL 防 Pod 泄漏（见 `07` §3）。
...
> 令牌体系、Prisma 全局过滤、RLS 策略的**完整实现**在 `05-认证与多租户账户.md` §6；本节把它上升为**平台级不可违反的红线**，并统一"跨租户/IDOR/需隐藏存在性的越权即 404"口径。RLS **强制 `FORCE`** 的决策与参考实现（会话变量 `app.tenant_id`/`app.user_id`、PgBouncer transaction 模式的 `SET LOCAL`、Prisma tenant extension 纵深）见**附录 C D2 / §5**。
...
2. **跨租户/IDOR/需隐藏存在性的越权一律返回 404，不返回 403。** 访问不属于当前租户的资源（会话/agent/文件/artifact/MCP/技能/分享）或同租户未授权 workspace 时返回 `404 Not Found`，避免泄露"该资源存在于别的租户/不可见工作区"这一存在性信息；同租户内角色/动作权限不足且无需隐藏存在性时才可用 `403 PERMISSION_DENIED`。
...
| 直接对象引用 IDOR | 资源 id 用不可枚举 UUID；查询必带 tenant_id；跨租户/需隐藏存在性的越权返回 404 | PEN-ISO-1 |
| 连接池串上一个租户的 `SET` | `SET LOCAL`（事务级）+ 每请求独立事务 | PEN-ISO-3 |
| 关掉应用层过滤仍越权 | RLS `FORCE` 兜底；应用连接用非 superuser、非 BYPASSRLS 角色 | PEN-ISO-4 |
| 缓存/Redis key 无租户前缀 | 所有 Redis key/channel 必须含租户段：普通缓存/限流用 `t:{tid}:...`，计费四桶用 `quota:{tid}:...`（见 `09`/附录 C D12），会话流 Redis Stream/PubSub 频道统一为 `stream:{tenantId}:{sessionId}`，用户级频道为 `stream:{tenantId}:user:{userId}`，runtime 路由/租约为 `rt:session:{tenantId}:*`、`rt:lease:{tenantId}:*`、`rt:workspace-lease:{tenantId}:*`、`rt:tenant:{tenantId}:count`，定时任务 BullMQ / 租约 / Worker 命令通道为 `sched:{tenantId}:*`、`sched:lease:{tenantId}:*`、`sched:worker:{tenantId}:*` | PEN-ISO-5 |
| 对象存储路径无租户前缀 | S3 key 强制 `tenants/{tid}/...`，服务端拼接，**绝不接受客户端传完整 key** | PEN-ISO-6 |
| 签名 URL 越权 | 对象级、限时效、绑定 `method + tenantId + previewSessionId + artifactId + path + exp + kid`（§5.4；不是 Cowork `sessionId`）| PEN-CONTENT-3/4 |
| OpenClaw 工作区跨租户复用 | 每会话独立 Pod + 每租户 PVC 的工作区子路径 `tenants/{tenantId}/ws-{workspaceId}`（挂载/租约单位为目标工作区；旧 `workspace-{agentId}` / `workspace-main` 只作迁移映射来源，见 `07` 与附录 C D5）；默认预热路径只预热节点/镜像/runtime/只读缓存，会话 Pod 创建时声明 PVC/ConfigMap/Secret，回收即销毁 | PEN-ISO-7 |
...
| 出站源 | 约束 | 主责 |
|---|---|---|
| 模型/ASR 代理 | 只出到受控上游模型/ASR 端点；Pod/浏览器不持真实 key（§6.4）| `09` |
...
| 绑定与规范化串 | 签名对**固定顺序规范化串**计算：`v1\n{method}\n{tenantId}\n{previewSessionId}\n{artifactId}\n{path}\n{exp}\n{kid}`，`HMAC-SHA256`，密钥仅后端持有；`previewSessionId` 是预览网关短期授权 ID，不是 Cowork `sessionId`，不得进入对象存储前缀；字段顺序/分隔符固定，禁止乱序或省略以防歧义签名 |
...
flowchart TB
  ...
  L1 --> L2[L2 签名URL 绑定 tenantId/previewSessionId/artifact/path/过期]
...
- 沙箱 Pod **不持有真实上游模型 key**，浏览器/沙箱也不持有真实 ASR provider key：模型调用改指向**内网模型代理 Service**，Pod 仅拿会话级短期凭证；ASR 调用只能经后端 ASR 代理的短期 stream ticket（对齐 `07` §5.3/§5.4、`09`）。这样单个沙箱或浏览器会话被攻破不会泄露平台上游 key。
...
| 面 | 威胁 | 防护 |
|---|---|---|
| REST/WS 入参 | 类型污染、超长、非法枚举 | 使用 `libs/shared/contracts` 生成的 Zod/validator 做白名单校验（可包装为 Nest pipe，见 `04`）；拒未知字段，禁止另起 `class-validator` DTO 事实源 |
...
2. **风险分级门控**（分清「单条 finding severity」与「聚合 risk level」两层，勿混用，见附录 C A6 / `10` §5.3）：
   - **单条命中 severity（权重）**：info=0 / warning=5 / danger=20 / critical=50（`skillSecurityScanner.ts:53-58`），既参与聚合打分，也作为 SaaS 目标态的硬门输入。
   - **聚合风险 level** 由 `riskScoreToLevel(总分)` 产出；单条 `critical` severity finding（如 data-exfiltration）只有 50 分，聚合 level 可能只是 `high`，所以**不能只看聚合 level**。
   - **目标态门控**：
     - 出现任一 `critical` severity finding（如 `data-exfiltration` / `obfuscated-code` / `suspicious-file`）或聚合 `riskLevel='critical'` → 默认 `blocked`，普通租户不可自助放行；仅企业租户管理员可强制放行且必记审计。
     - 无 critical finding，但存在 `danger` finding 或聚合 `riskLevel ∈ {medium,high}` → 需用户显式确认后 `ready`，否则保持 `blocked`。
     - 仅 `info/warning` 且聚合 `safe/low` → 自动 `ready`。
   - 订正：附录 C A6 只是在纠正“把现状 `riskScoreToLevel(50)='high'` 误读成聚合 critical”的事实错误；SaaS 目标门控仍必须新增 finding 级 severity 硬阻断，不能退回“单条 data-exfiltration 可由普通租户确认启用”的旧口径。
...
| 并发沙箱 Pod | 按套餐（免费 1 / 标准 5 / 企业可配）| Redis `rt:tenant:{tenantId}:count`（`07` §6.2）|
...
| 存储 | 每租户/工作区配额，超限 `507 STORAGE_QUOTA_EXCEEDED`（`413` 仅用于 `PAYLOAD_TOO_LARGE`，账单额度仍用 `QUOTA_EXCEEDED`） | 文件服务（`08` §8）|
...
> **版本边界**：本节 §11.2/§11.3 定义的是 V6 GA 的完整合规目标。V3 Beta 只要求 `17` V3-E6 的「请求受理 + 审计记录 + 支持 runbook + 人工/后台处理」最小闭环，不要求自动生成完整可携带导出包、六子系统级联删除或签名删除凭证；但 PR-1/附录 C 已冻结 endpoint/字段骨架，避免后续重命名。
...
```mermaid
flowchart LR
  U[租户/用户请求导出] --> API[POST /api/v1/privacy/exports]
  API --> Q[BullMQ 导出任务]
...
- **端点与幂等**：`POST /api/v1/privacy/exports` 以 `exportId` 幂等——重复请求返回同一进行中/已完成任务；如需查询进度，使用资源化路径 `GET /api/v1/privacy/exports/{exportId}`。禁止把正式 REST 写成动词路径 `/api/v1/privacy/export`。字段级契约以 `libs/shared/contracts` 为权威（见附录 C D1）。
...
flowchart TB
  ...
  HARD --> D3[PVC: 卸载并删除该租户 PVC/卷子树]
  HARD --> D4[Vault: 销毁该租户全部 secref/BYOK]
  HARD --> D5[Redis/缓存: 清所有含 tid 的 t/quota/stream/rt/sched key]
  HARD --> LEGAL[账务/审计: 匿名化保留(法定义务)]
  LEGAL --> CONFIRM[出具删除完成凭证]
```

- **REST 端点**（字段级契约以 `libs/shared/contracts` 为权威，见附录 C D1）：
  - `POST /api/v1/privacy/deletions`——发起软删除，body `{ scope:'tenant'|'user', subjectId, reason }`，返回 `{ deletionId }`；
  - `GET  /api/v1/privacy/deletions/{deletionId}`——查询进度与各子系统部分失败明细。
  - 禁止把正式 REST 写成动词路径 `/api/v1/privacy/delete` 或 `/api/v1/privacy/delete/:deletionId`；OpenAPI 源文件使用 `{deletionId}`，文档 shorthand 也优先写 `{}` 以免 contracts 误抄。
- **六子系统级联顺序（固定，先断访问面再删数据）**：① 撤销令牌/会话（`05` §4.2）→ ② Redis 缓存/路由清 `t:{tid}:*` / `quota:{tid}:*` / `stream:{tid}:*`，以及 runtime 路由/租约 `rt:session:{tid}:*`、`rt:lease:{tid}:*`、`rt:workspace-lease:{tid}:*`、`rt:tenant:{tid}:count` 和定时任务 `sched:{tid}:*`、`sched:lease:{tid}:*`、`sched:worker:{tid}:*` 等含租户段 key/channel → ③ Vault 销毁全部 secref/BYOK → ④ 对象存储删 `tenants/{tid}/` 全前缀（含 shares/exports/backups）→ ⑤ PVC 卸载并删除该租户 PVC；若存储实现不是独立 PVC 而是共享底层卷上的租户子树，则必须删除 `tenants/{tid}/` 子树并清理 PV/PVC 引用 → ⑥ Postgres 业务行级联删；审计做**内容匿名化保留**（法定义务，不进物理级联删）。
...
### 11.6 商业 / 合规 / 运维延后项责任矩阵

这些项看似“上线前再补”，但都会反向影响数据模型、部署拓扑、权限、审计和 UX。下表是架构层 owner 与版本归属；未完成对应版本门时，不得以“运营流程后补”为理由上线。

| 项 | 默认 owner | 版本归属 | 架构影响 | 最低验收证据 |
|---|---|---|---|---|
| 支付 / 充值 / 订阅 webhook | 计费负责人 + 财务 | V5 付费试点 | `billing_ledger` 幂等、支付方 webhook 签名校验、`refund` 必带 `reason=payment_refund|chargeback` 且负向 clawback、订阅状态机；影响 `09` billing API 与审计 | 支付沙箱端到端、重复 webhook 不重复入账、退款/拒付对账报告含 ledger reason 与负向 bucketDeltas |
| 发票 / 税务 / 地区税率 | 财务/法务 + 计费负责人 | V5 设计，V6 GA 完整 | 发票抬头/税号/地区税率字段、账单保留期、删除权合法保留；影响 `09`、本节 §11.1 | 月账单与 ledger 聚合一致，发票样张和税务保留策略签收 |
| 数据驻留 / 区域绑定 | 合规/法务 + 架构/PLAT | V3 设计，V6 GA 准入 | `tenant.region`、DB/S3/K8s 节点池按 region 分片、路由与 config sync 区域选择；影响 `02/05/06/08/15` | 两区域租户数据不跨区落盘的审计报告，跨区聚合脱敏说明 |
| SLA / status page / 客户支持 | SRE/PM + 客服负责人 | V6 GA | SLI/SLO 数据源、组件级合成监控、状态页事件模型、赔偿 credit 与计费对账；影响 `15` 与 `09` | 状态页演练、P1 工单到 on-call 流程、SLA 违约计量样例 |
| kill-switch / feature flag 控制面 | SRE/SEC + 平台负责人 | V5 前可用，V6 演练 | 集中 flag 存储、按租户/套餐/provider/能力位控制、≤60s 生效、RBAC + 双人复核；影响 `15`、`04` 错误信封、前端门禁 | 任一高风险能力 60s 内停用演练，审计记录完整 |
| 审计日志保留与匿名化 | 安全负责人 + 合规/法务 | V3 起采集，V6 策略签收 | 审计日志独立存储、保留期、不可抵赖、删除权下匿名化保留；影响 `06` audit schema、`15` Loki/存储策略 | 审计查询样例、脱敏验证、保留/清理任务报告 |
| ToS / AUP / 内容政策版本同意 | 法务/PM + Auth 负责人 | V2 注册登录前置，V6 文本完整 | `users.terms_version`、`terms_accepted_at`、政策版本升级再同意；影响 `05/06` 身份表与前端登录 UX | 新用户注册同意记录、政策升级后强制再同意用例 |
| 数据导出 / 删除权凭证 | 合规/法务 + DATA/BE | V6 GA | 导出包生成、删除任务状态机、六子系统级联、签名删除凭证；影响 `06/08/14/15` | 导出可下载、删除任务全链路演练、凭证可校验 |
...
| PEN-ISO-5 | 检查 Redis/缓存/Pub-Sub key 前缀 | 所有 Redis key/channel 均含租户段：普通缓存/限流为 `t:{tid}:...`，计费为 `quota:{tid}:...`，会话流为 `stream:{tenantId}:{sessionId}`，用户级事件为 `stream:{tenantId}:user:{userId}`，runtime 路由/租约为 `rt:session:{tenantId}:*`、`rt:lease:{tenantId}:*`、`rt:workspace-lease:{tenantId}:*`、`rt:tenant:{tenantId}:count`，定时任务 BullMQ / 租约 / Worker 命令通道为 `sched:{tenantId}:*`、`sched:lease:{tenantId}:*`、`sched:worker:{tenantId}:*` |
| PEN-ISO-6 | 尝试传入完整/他租户 S3 key、签前缀级 URL | 拒绝；key 仅服务端按 `ctx.tid` 拼接 |
| PEN-ISO-7 | 租户 A 沙箱内读/写租户 B 的 `tenants/{tenantId}/ws-{workspaceId}` PVC 子路径，或读/写同租户未授权 workspace | 越权读写被拒；文件/API 场景统一 404 不泄露存在性，Pod/PVC 探针无读写权限；同 workspace 多会话共享文件只校验 D5 写者租约，不要求跨会话不可见 |
...
| PEN-CONTENT-4 | 篡改签名 URL 的 `tenantId` / `previewSessionId` / `artifactId` / `path` / `method` / `kid` | 返回 403；无法越权取他租户对象，也不能复用其它预览会话授权 |
...
| 编号 | 测试动作 | 通过判定 |
|---|---|---|
| PEN-SUP-1 | 安装单条 data-exfiltration 命中的 skill | 虽然聚合 `riskScoreToLevel(50)` 仅得 `high`，但 data-exfiltration 属 `critical` severity finding；目标态按 finding 级硬门判为 `blocked`，普通租户不可自助启用，仅企业租户管理员强制放行并审计（见附录 C A6 / `10` §5.3）|
...
| PEN-SECRET-3 | 从沙箱 Pod / 浏览器 ASR 会话尝试获取真实上游模型或 ASR key | 拿不到（Pod 仅持会话级短期凭证，ASR 仅持一次性 stream ticket）|
...
| PEN-ABUSE-2 | 超并发 Pod / 超配额写入 | 超并发 Pod 优雅排队/限流；超存储配额写入返回 `507 STORAGE_QUOTA_EXCEEDED`（非 `413`），不影响他租户，不打爆 K8s API |
...
- 认证、令牌、RLS、RBAC、跨租户/IDOR/需隐藏存在性=404 的完整实现：`05-认证与多租户账户.md`
...
- MCP/技能供应链、`mcpCommandPolicy`、扫描引擎与门控流程：`10-MCP与技能改造.md`
