...
**现状根因。** 目标是「几乎所有功能都要 web 化」，但已明确 V1-V6 GA 主线 = 核心对话/Agent/Artifacts/Skills/MCP + 文件工作区读写 + 定时任务 + 认证多租户 + 模型代理与计费；IM 渠道和第三方 OAuth 代持为 GA 后续，computer-use/VM/后台浏览器不做。诱惑在于这些功能代码已存在（`src/main/im/`、`src/main/computerUse/`、浏览器自动化），容易被「顺手也做了」回灌，冲垮进度与隔离预算。
...
风险不是登记完就结束，而是绑定到路线图（`17`）的阶段准入门。**阶段门命名统一采用附录 C §8 唯一对照表的 V1–V6（D13），废弃旧的 P0–P3 / GV 别名**；每条风险绑定到对应版本门，越权、迁移双读、egress 等门的版本时点以附录 C §8 为准。任一版本门的 🔴 风险缓解未达标，不得进入下一版本门。

```mermaid
flowchart LR
    V1[V1 可行性验证] -->|G-V1 门<br/>R-OC-03/ISO-01/ISO-04/OC-01| V2[V2 单租户Web闭环]
    V2 -->|G-V2 门<br/>R-STREAM-01 + 10事件契约| V3[V3 多租户Beta]
    V3 -->|G-V3 门<br/>R-SEC-01/DATA-01 硬门| V4[V4 运行时强化]
    V4 -->|G-V4 门<br/>R-PERF-01/03 + R-ISO-02| V5[V5 商业化生态]
    V5 -->|G-V5 门<br/>R-COST-01/ISO-03/SEC-03| V6[V6 GA运营]
    V6 -->|G-V6 门<br/>R-COMP-01 + P0/P1清零| GA[GA 上线运营]
```

| 阶段门 | 必须清账的风险 | 判据（详见附录 C §8） |
|--------|----------------|------|
| **G-V1** 可行性验证 | R-OC-03、R-ISO-01、R-ISO-04、R-OC-01、R-STOR-01、R-SEC-04、R-OPS-01 | 真实 gVisor Pod 跑通一条 turn；**gVisor 工作负载兼容+性能矩阵达标（不达标路由 Kata，见 R-ISO-04 / 附录 C D4）**；RWX/PVC+S3 存储 PoC；桥契约冻结；Config Sync golden；模型出网封锁 PoC；镜像可复现且不含 noVNC/Xvfb/Electron UI/真实密钥/租户数据；patch 全 apply + 回归绿；逃逸测试 100% 拦截；K8s PoC 跑通 |
| **G-V2** 单租户 Web 闭环 | R-STREAM-01 | 登录→发消息→流式→artifact 打通；契约测试（10 事件全覆盖）；单租户集成绿；WS 断连续传达标 |
| **G-V3** 多租户 Beta | R-SEC-01、R-DATA-01 | Tenant/RBAC/**RLS 强制**；**跨租户越权必过（硬门）**；**SaaS schema 迁移双读校验（硬门）**；工作区 API；服务端 BullMQ 定时任务 |
| **G-V4** 运行时强化 | R-ISO-01、R-ISO-02、R-PERF-01、R-PERF-03 | Orchestrator/每会话 Pod/gVisor+Kata/NetworkPolicy/egress/预热/自愈；并发压测、沙箱逃逸、冷启动、自愈演练；Sandbox 实测资源模型已回写 requests/limits |
| **G-V5** 商业化生态 | R-COST-01、R-ISO-03、R-SEC-03、R-VENDOR-01 | 硬配额+熔断上线且**计费不可绕过**；**stdio MCP 不在宿主执行**；MCP/Skills 供应链门控；公开分享 abuse 治理；桌面存量导入双读（可选门）；provider 故障切换演练通过 |
| **G-V6** GA 运营 | R-COMP-01 + 全量 P0/P1 | 安全压测、混沌、告警回滚、SLO、合规生命周期；P0/P1 清零 |
