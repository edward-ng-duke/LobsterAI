...
模型网关（对应 `04` 的 **ModelProxyModule**，GA 主线可先作为 NestJS 模块，随负载拆独立服务）承担五职：
...
| 层 | 控制 | 验收 |
|---|---|---|
| 网络层 | sandbox namespace `NetworkPolicy` 默认拒绝出站，只放行模型网关 Service、对象存储必要端点、受控 MCP 出口；provider 公网域名和任意外网默认拒绝 | Pod 内 curl OpenAI/Anthropic/Gemini 公网域名失败并产生审计 |
...
3. 调用计费服务做 preauth，额度不足直接拒绝，不发上游请求。
...
model CreditLedger {
  ...
  entryType    String   @map("entry_type")      // preauth | settle | release | refund | grant | topup | adjust
  ...
  @@unique([tenantId, requestId, entryType])    // 幂等：含 tenant_id，跨租户不碰撞、重放不白嫖
  @@map("credit_ledger")
}
```

- `credit_ledger` 是**不可变追加流水**，账户余额 = 初始 + Σledger（或以 `credit_accounts` 做物化快照 + 定期对账，见 §8）。
- **幂等唯一键 = `(tenant_id, request_id, entry_type)`**（见附录 C D12 / §6）：保证「同一次调用的预扣/结算/释放」幂等，**且含 `tenant_id` 以防跨租户碰撞**；`request_id` 由**服务端签发**（不复用客户端上报值，避免重放白嫖），配合 `04` §4.5 的 `Idempotency-Key`。
...
- **退款 clawback**：退款/拒付走 `credit_ledger(entryType='refund')` 分录做 **clawback**（冲销已入账 credits，允许账户转负并在下次门控要求补足），与 `topup`/`settle` 同表对账（§8）。
...
|---|---|---|
| 预扣 | `preauth` | 按字段级公式估算 credits：`hold = ceil( (maxInputTokens_est × price_input + maxOutputTokens_est × price_output) ÷ 1000 × costMultiplier )`（`price_*` 为 credits/1K token，量纲同 §3.1，**须 `÷1000`**）；在 Redis **四桶按优先级 Lua 原子扣减**（§5.3），写 `preauth` 流水（占位） |
...
sequenceDiagram
  ...
  alt 四桶合计不足
    ...
    MGW->>PG: INSERT ledger(preauth, tid, requestId, estCredits)
    MGW->>UP: 转发(真实 key)
    loop 流式
    ...
    MGW->>PG: INSERT ledger(settle, tid, requestId, realCredits)
...
- **幂等**：`(tenant_id, request_id, entry_type)` 唯一约束（§3.3）保证网络重试/补发不重复扣费；**`request_id` 由服务端签发**（不复用客户端上报值，避免跨租户碰撞与重放白嫖），配合 `04` §4.5 `Idempotency-Key`。
...
model MediaTask {
  id            String   @id @default(uuid())
  taskId        String   @unique @map("task_id")     // 对外统一 text 类型（前端/轮询用）
  tenantId      String   @map("tenant_id")
  sessionId     String?  @map("session_id")
...
- [ ] 每次模型调用产生成对 `preauth`/`settle` 流水，`(tenant_id, request_id, entry_type)` 幂等（重试不重复扣费；`request_id` 服务端签发）。
...
§3–§8.4 保证「算得准、扣得对」；能否**赚钱**取决于单位经济性——每个租户 / 每次调用的**收入 − 真实成本**是否为正。本节只承接口径要点，字段级模型与阈值属大块新增设计（记入 deferred，落地见 `17`/`15` 与本附录同源）：
...
额度账户快照（**兼容**现状 `normalizeAuthQuota` 形态，`main/authQuota.ts`）——注意：现状 `normalizeAuthQuota` 是 if/else 链、**只产单一 `creditsLimit`**，而此处新增 `breakdown` 四桶明细，语义已变，**「前端零改」不成立**（见附录 C C19）；顶层 `creditsLimit/Used/Remaining` 仅作兼容兜底，前端需改造以消费四桶明细与新订阅态：
...
| ledger 重复扣费 / 跨租户碰撞 / 重放白嫖 | 账目错误 | `(tenant_id, request_id, entry_type)` 幂等（`request_id` 服务端签发）+ `Idempotency-Key`（§3.3/`04` §4.5、附录 C D12） |
