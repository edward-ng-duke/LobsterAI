...
| `/auth/login` | POST | email+密码登录，返回一次性授权码 `{ code, state }`；**不直接建会话、不下发 token、不返回 302**，后续统一经 `/oauth/token` 换 token |
| `/oauth/authorize` | GET | OAuth2 授权端点（Authorization Code + PKCE） |
| `/oauth/token` | POST | 用 code+verifier 换 access/refresh；移动端/非浏览器客户端才在此处理 `grant_type=refresh_token` body |
| `/auth/callback` | GET | 联邦 SSO（Google/GitHub/SAML）后端回调；换取 IdP userinfo 后签发内部授权码并回跳 SPA，不在该端点直接下发 token |
...
所有端点名用共享 route registry / 常量集中管理（遵循根 `AGENTS.md` 字符串常量规范）。PR-1 起权威应落在 `libs/shared/contracts` 的 route metadata / OpenAPI 源；现有 `src/shared/auth/constants.ts` 只作为初始抽取输入，不能作为目标态认证契约事实源。示例：

```ts
// libs/shared/contracts/src/auth-routes.ts（或等价 route metadata 源）
export const AuthRoute = {
  Register: '/auth/register',
  Login: '/auth/login',
  OAuthAuthorize: '/oauth/authorize',
  OAuthToken: '/oauth/token',
  FederatedCallback: '/auth/callback', // auth.<domain> 后端 IdP callback；不要和 app.<domain> SPA 路由混入同一服务端 registry
...
> **`/auth/callback` 双宿主边界**：`auth.<domain>/auth/callback` 是 Auth 服务接外部 IdP 的后端回调；`app.<domain>/auth/callback` 是 SPA 路由，用于接内部授权码并调用 `/oauth/token`。两者可同名但 host/owner 不同，PR-1 route registry 必须把服务端 AuthRoute 与前端 SPA route 分开，不能把 SPA route 当成 NestJS AuthModule 端点，也不能把后端 IdP callback 挂到前端路由。
...
| 本机 HTTP server 收 code | SPA 路由 `/auth/callback` 收 code，POST 到 `/oauth/token` 换 token |
...
sequenceDiagram
  ...
  B->>IdP: 用户在 Google 授权
  IdP-->>A: 302 auth.<domain>/auth/callback?code&state（回到 A，不是回浏览器）
  A->>IdP: 用 code 换 IdP token, 拉 userinfo(email)
  A->>A: 按 email 匹配/新建 User + IDENTITY(provider=google)
  A-->>B: 302 回 SPA app.<domain>/auth/callback?code=内部授权码&state
  Note over B,A: 之后与 3.2 相同：POST /oauth/token 换内部 access/refresh
```

- 外部 IdP 回调落在**后端** `auth.<domain>/auth/callback`（不是 SPA 路由），后端换取 userinfo 后再签发**内部**授权码回到 `app.<domain>/auth/callback`，统一收敛到 3.2 的令牌签发路径。
...
> 契约测试从 `libs/shared/contracts` 的 Auth route registry / OpenAPI paths 穷举端点，逐端点断言 request/response/error 三段齐备（见附录C D1）；PR-1 初次抽取时可把现有 `src/shared/auth/constants.ts` 的 `AuthRoute` 作为比对输入，防止漏迁移，但稳定态不得反向依赖 `src/shared`。
...
| 直接对象引用（IDOR） | 资源 id 用不可枚举 UUID；查询必带 tenant_id；跨租户/需隐藏存在性的越权返回 404 |
| 连接池串上一个租户的 `SET` | 用 `SET LOCAL`（事务级），或每请求独立事务 |
| 缓存/Redis key 无租户前缀 | 所有 Redis key/channel 必须含租户段；普通缓存/限流用 `t:{tid}:...`，计费四桶用 `quota:{tid}:...`，广播频道用 `stream:{tenantId}:...`（流式见 `03`，计费见 `09`） |
| 对象存储路径无租户前缀 | S3 key 强制 `tenants/{tid}/...`（见 `08`） |
| OpenClaw 工作区跨租户复用 | 每会话独立沙箱 Pod + 每租户 PVC 的工作区子路径 `tenants/{tenantId}/ws-{workspaceId}`（挂载/租约单位为目标工作区；旧 `workspace-{agentId}` / `workspace-main` 只作迁移映射来源，见 `07` 与附录 C D5） |
...
| WebSocket 鉴权 | 连接建立后首帧只用短寿命一次性 ticket：先由 REST access token 调 `POST /api/v1/stream/ticket`，OpenAPI 请求体字段固定为 `sessions[]` 与 `resourceSubscriptions[]`；ticket 内部绑定 `tenantId/userId`、由 `sessions[]` 派生的授权会话集合、`resourceSubscriptions/expiresAt/nonce`；`resourceSubscriptions` 至少覆盖 `files:changed` 的 `workspaceId/path` scope，用户级事件随 auth 自动恢复；首帧消费；不把 JWT 放 URL query，也不把长效 Bearer 作为正式 WS 鉴权；见 `03-前端与传输层改造.md` |
...
> 完整事务邮件投递（模板、退信、供应商、DKIM/SPF）不是无主后置项：按 §9 落在 V5-E6 前补齐，由 Auth/BE + SRE 负责并对齐 `15-部署运维与可观测性.md` 与 `libs/shared/contracts`。V2/V3 若尚未接真实邮件供应商，只能用于内部 Alpha/邀请制 Beta 的受控账号流（例如 OIDC、管理员预创建或开发邮件捕获），不得宣称自助邮箱验证/忘记密码/重置密码完整可用；一旦开放 email+password 自助注册或重置，邮件投递、退信/频控和 DKIM/SPF 必须同步进入验收。
...
| 版本 | 步骤 | 依赖 |
| --- | --- | --- |
| V2-E1 | 建身份表（字段级 DDL 见**附录C §4**，`06` 为叙述 owner，见 D3）；Argon2id；JWKS | 附录C §4 / `06` |
| V2-E1 | AuthModule：register/login/token/refresh/logout + Guard 链 | `04` |
| V2-E4 | SPA 登录页 + `/auth/callback` 路由 + PKCE + 内存/Cookie 令牌 | `03` |
| V2-E2 / V3-E1 | 现有业务表加 `tenant_id` + Prisma tenant extension + 强制 RLS 策略 | `06` |
| V2-E1 / V3-E1 | 中间件把 `tid`+`user_id` 注入 RLS 会话变量；BullMQ/WS 带租户上下文 | `04`/`03` |
| V5-E6 | 事务邮件端点（verify-email/forgot/reset）+ `auth_email_tokens` 表 | `04`/`15` |
| V3-E1 / GA 后续 | 租户切换、成员邀请进入 V3；第三方联邦 SSO（Google/GitHub）按 `13` 列 GA 后续 | — |
| V5-E1 | 配额/套餐挂租户，与计费服务对接 | `09` |
| V6-E1 / V6-E5 | MFA/step-up、审计日志、superadmin 后台 | `14` |
...
- [ ] `/api/v1/stream/ticket` 只能由有效 access token 申请；ticket TTL 30-60s、一次性消费、绑定租户/用户/会话订阅与资源订阅（如 `files:changed` workspace scope），过期、复用、跨租户 session 或未授权 workspace scope 均失败且不泄露存在性。
...
- [ ] Redis key/channel 均含租户段（普通缓存 `t:{tid}:...`、计费 `quota:{tid}:...`、流式 `stream:{tenantId}:...`）；对象存储 key 均带 `tenants/{tid}/` 前缀。
...
| 风险 | 影响 | 缓解 |
| --- | --- | --- |
| tenant_id 漏过滤导致越权 | 严重（数据泄露） | 双层（应用 extension + RLS FORCE）+ 红队 + 跨租户/IDOR 越权返回 404 |
