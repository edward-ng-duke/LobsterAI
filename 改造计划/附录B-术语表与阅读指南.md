...
> 本文档是整套「LobsterAI 桌面端 → 多租户 SaaS Web 应用」改造计划的**词典与导航图**。它有两个用途：（1）作为**术语表**，统一全套文档中反复出现的名词定义，避免同一个词在不同分册被不同理解；（2）作为**阅读指南**，给出 21 篇文档的一句话索引，并按角色给出推荐阅读顺序。适合读者：**所有人**——尤其是新加入项目、第一次翻开这套文档的读者，建议先读 `00-总览与执行摘要.md` 再回到本文建立词汇基线。
...
| **gateway（网关）** | 有两个含义，需按上下文区分：（1）**OpenClaw gateway** —— OpenClaw 运行时进程本身，现状监听 `ws://127.0.0.1:{port}`、token 鉴权、维护 state dir 与每 agent 文件工作区；定时任务的权威 cron 也在这里。（2）**API 网关 / BFF** —— 目标架构接入层的 NestJS 服务，负责 HTTP 路由、WS 升级与广播、JWT 校验、限流。除非明确写「API 网关」，术语「gateway」通常指前者（OpenClaw gateway）。 |
...
| **记忆 / Dreaming（记忆子系统）** | LobsterAI 的长期记忆与「梦境」整理机制：`user_memories`/`user_memory_sources`/`cowork_user_memories` 表 + 工作区 `MEMORY.md` + dreaming（`cowork:dreaming:*`）后台整理。多租户下存储归属见 `06`，调度归属见 `07`/`11`。 |
...
| **多租户（multi-tenancy）** | 多个租户共享同一套后端与集群、靠 `tenant_id` 逻辑隔离的部署形态。v1 默认「共享库 + `tenant_id` 列（+ 可选 RLS）」，非「每租户独立库」。见 `02`、`06`。 |
| **RLS（Row Level Security，行级安全）** | PostgreSQL 的行级访问控制：即使应用代码漏写 `where tenant_id`，数据库层也能拦截跨租户读写。作为纵深防御（defense in depth）兜底。需在连接上 `SET app.tenant_id` 并注意连接池会话变量清理。见 `06`、`14`。 |
...
| **上下文用量 / 压缩（context usage / compaction）** | 会话累计消耗的上下文窗口占用统计与自动压缩机制；流式事件 `cowork:stream:contextUsage` 推送用量。见 `04`。 |
...
| **scheduled_tasks / scheduled_task_runs（历史遗留表）** | SQLite 中的定时任务与运行历史表，属**历史遗留**：定时任务的真正权威是 **OpenClaw gateway 的 cron**，这两张表只在迁移逻辑里被读出、迁入 gateway 后即废弃（以 `11-定时任务调度.md` 口径为准）。 |
...
| 术语 | 定义 |
|---|---|
| **window.electron（唯一桥）** | 现状渲染层↔主进程的**唯一** contextBridge 桥（`src/main/preload.ts`）。约 446 处调用、约 74 个渲染文件，收口于 `src/renderer/services/*`。是整个 Web 化的最大杠杆点。见 `03`、`附录A-IPC通道与接口映射.md`。 |
...
| **IPC（Inter-Process Communication）** | Electron 主进程与渲染进程间的通信。现状请求走 `ipcRenderer.invoke` / `ipcMain.handle`，流式走主进程 `webContents.send` + 渲染层 `ipcRenderer.on`。`src/main/main.ts` 约 10560 行、259 个 `ipcMain.handle/on`；`webContents.send` 调用点约 47 处、去重后事件通道约 29 个（引用时区分「调用点数 ≈47」与「去重通道数 ≈29」）。见 `01`、`附录A-IPC通道与接口映射.md`。 |
| **`cowork:stream:*`** | Cowork 对话的流式事件通道族（如 `messageUpdate`/`permissionRequest`/`contextUsage`/`complete`）。SaaS 下映射到 WS 订阅，经 Redis pub/sub 跨网关实例广播。见 `03`。 |
...
| **WebSocket（WS）** | 目标架构里对应「服务端主动多次推送」语义的传输（顶替 `webContents.send` 系列）。需处理跨实例广播、断线重连补发、`requestId` 多路复用、连接鉴权。见 `03`。 |
...
| **warm pool / 预热池** | 预先创建并保持就绪的沙箱 Pod 池，用于降低每会话冷启动延迟。编排器优先从池取 Pod，miss 才新建。见 `07`。 |
| **冷 / 温 / 热三态** | 沙箱资源的生命周期状态：冷（无 Pod，需新建，首字延迟最高）、温（预热池就绪待分配）、热（已绑定会话可直接复用）。用于在体验与成本之间平衡。见 `07`、`15`。 |
...
| 术语 | 定义 |
|---|---|
| **Kubernetes（K8s）** | 容器编排平台，承载全部后端服务与沙箱 Pod 池。见 `02`、`07`、`15`。 |
...
| **BullMQ** | 基于 Redis 的分布式任务队列，承担定时任务触发与异步作业（工作区快照 / 技能安装等），提供延迟任务、重试、并发控制。见 `11`。 |
...
| 术语 | 定义 |
|---|---|
| **IM 渠道** | 即时通讯接入（微信 / 飞书 / 钉钉 / QQ / Telegram / Discord / 邮件 / NIM / POPO / WeCom 等）。多为 OpenClaw connector 常驻连接（`src/main/im/`）。**v1 后续再做**，不阻塞主线。见 `13`。 |
...
全计划共 **21 份文档**（19 篇正文 + 2 篇附录）。下表每篇一句话摘要 + 最相关的交叉引用；文件名与仓库中完全一致。
...
| 06 | `06-数据模型迁移.md` | SQLite→Postgres 多租户迁移：全表补 `tenant_id`、Prisma schema、迁移脚本、可选 RLS。 | 05、11、14 |
| 07 | `07-OpenClaw运行时编排与沙箱隔离.md` | **最难一章**：每租户 / 会话沙箱 Pod、gVisor/Kata、PVC、预热池 / 租约 / 冷温热三态。 | 08、14、02 |
...
| 11 | `11-定时任务调度.md` | 定时任务调度：以 OpenClaw gateway cron 为权威，BullMQ 触发，历史表迁移即废弃。 | 06、07、04 |
...
| 17 | `17-分阶段路线图与工作量估算.md` | M0–M9 里程碑、任务分解、人月估算与团队规模。 | 00、07、18 |
...
| 2 | `13-功能取舍与降级清单.md` | v1 做 / 不做 / 降级边界，管理外部预期。 |
...
| 6 | `11-定时任务调度.md` | cron 权威归属、BullMQ 触发、历史表迁移。 |
...
| 顺序 | 文档 | 关注点 |
|---|---|---|
| 1 | `07-OpenClaw运行时编排与沙箱隔离.md` | Pod 编排、gVisor/Kata、PVC、预热池 / 租约 / 三态。 |
...
> 小结：本文是全套文档的**入口词典 + 导航图**。第 2 节统一术语，第 3 节一句话索引 21 篇，第 4 节按角色给路线，第 5 节收束三条硬主线。遇到任何不确定，先回本文对齐词汇，再回 `00`/`02` 对齐全局。
