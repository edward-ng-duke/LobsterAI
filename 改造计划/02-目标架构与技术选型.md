...
| V1-V6 GA 主线范围 | 核心对话 / Agent / Artifacts / Skills / MCP + 文件工作区读写 + 定时任务 + 认证多租户 + 模型代理与计费；其中第一版 V1 是可行性验证版，不是商业 v1 | 见 `13-功能取舍与降级清单.md`、`17-分阶段路线图与工作量估算.md` |
...
> 浏览器里的 **React SPA**（复用现有代码，靠「浏览器桥」顶替 `window.electron`）通过 **REST + WebSocket** 访问一组按域拆分的 **NestJS 后端服务**；后端把原来跑在用户机器上的 **OpenClaw gateway** 迁移到 **Kubernetes 上按会话/租户隔离的沙箱 Pod 编排体系**里运行，并用预热容量降低冷启动；状态从单机 SQLite/文件系统迁移到 **PostgreSQL + Redis + S3 兼容对象存储**。
...
| `src/main/main.ts`（约 **11307 行**；`main.ts` 内 `ipcMain.handle` 211 + `on` 6 = **217**，全 `src/main` 合计 ≈283；`.send(` 调用点 ≈51、`webContents.send` ≈36，去重事件通道 ≈29） | 拆成 8 个后端服务模块 | `04`、附录C §2-B9/B14/B15 |
| 本地 OpenClaw gateway（`src/main/libs/openclawEngineManager.ts`，`ws://127.0.0.1:{port}` `openclawEngineManager.ts:344`，本地 state dir + 工作区） | K8s 沙箱 Pod 编排体系（每会话/租户一个 Pod，gVisor/Kata 加固，租户 PVC 工作区） | `07`、`08` |
...
flowchart TB
  ...
  subgraph Runtime["运行时隔离层 (Kubernetes)"]
    direction TB
    PodPool["OpenClaw 沙箱 Pod 编排<br/>每会话/租户一 Pod<br/>gVisor / Kata 加固"]
...
flowchart TB
  ...
  subgraph K8s["Kubernetes 集群 (单区域, 多可用区)"]
    ...
    subgraph SandboxNS["sandbox 命名空间（强隔离，独立节点池）"]
      ...
      Pods["OpenClaw 会话沙箱 Pod<br/>per-session / per-tenant"]
      NodePool --- Pods
    end
...
| `sandbox` | OpenClaw 会话沙箱 Pod 编排体系 | **专用节点池** + gVisor/Kata `RuntimeClass`；`NetworkPolicy` 默认拒绝出网，仅放行到模型网关；不可访问 `app`/`data` 内部服务。详见 `07`、`14` |
...
> 落地建议：以上服务放在**一个 NestJS monorepo**（`apps/gateway`、`apps/cowork`、`apps/orchestrator`…）中，共享 `libs/`（复用现有 `src/shared/*` 与从 `src/main/libs/*` 抽取的纯 TS 业务逻辑）。初始交付可先合并为「网关 + 单体应用」，随负载再拆分独立部署单元——服务边界先定清楚，物理拆分可延后。拆分粒度与 API 契约见 `04-后端服务与API设计.md`。

> **域归属补充（Agent 域 / IM 域）**：上表 8 服务未单列「Agent 域」与「IM 域」，其归属明确如下，避免 agent 误判要不要单独建服务：
...
| **为什么** | 渲染层有实测 **481 处 `window.electron` 调用、72 个文件**，且**并未全部收口于 `src/renderer/services/*`**（组件层直连 245 处 > services 207 处，过半绕过 services，见附录C §2-A4/B10）。因此浏览器桥必须 **1:1 复刻整个 `window.electron` 全局表面**（而非只替换 services 层），才能让业务组件**零改动**、改造集中在桥这一层。Vite 已是现有构建工具，无需换。 |
...
| 项 | 内容 |
|---|---|
| **推荐** | **Kubernetes**；每用户/每会话一个**沙箱 Pod** 跑 OpenClaw gateway，以 **gVisor 为默认 runtime**、**Kata Containers 为「重/兼容」资源档回退**（`runtimeClassName` 由编排器按 `resourceClass` 注入，见附录C D4）做内核级加固；每租户一个 **PVC** 作为工作区。 |
...
| **取舍** | GA 主线用 Helm + Actions 推送即可；镜像分层构建（后端 monorepo 各服务共享基础层），OpenClaw 运行时镜像单独构建并 pin 版本（现状 pin `v2026.6.1`，见 `package.json` `openclaw.version`）。详见 `15`。 |
...
- **接口契约事实源 = OpenAPI 3.1（REST）+ AsyncAPI 2.6（WS 流）**（附录 C D1）。契约物理落在 `libs/shared/contracts/`（`openapi.yaml` + `asyncapi.yaml`，或等价 `zod` schema），由 CI 生成后端 DTO 校验、前端桥类型、契约测试三方代码；**附录 A / `04` 降级为「导航与任务清单」，字段级一律以 `libs/shared/contracts` 为权威**。流式事件全集须从 `src/shared/*/constants.ts` 的 `as const` 对象穷举导出（**禁止用字面量 grep 数通道**——此前因此漏掉 `cowork:stream:goal`，真实为 10 个流事件，见附录 C §3.2/§2-B13）。
...
sequenceDiagram
  ...
  B->>GW: REST 申请一次性 WS ticket<br/>(scope含tenant/user/session/subscriptions)
  ...
  GW-->>B: {type:'authok'}
  B->>GW: subscribe {sessionId, sinceSeq?}
...
| 复用现有 TS 逻辑夹带 Electron 依赖 | 后端无法运行 | 复用按文件分级（lift-as-is / 需适配 / 必须重写）；`coworkStore`/`openclawConfigSync` 属「必须重写/移植」而非直接搬（各 15–25pd，见附录C D15），其余抽 `libs/` 纯逻辑并剥离 `electron-*` | `04`、`17` |

---

> 小结：目标架构 = **复用前端 + 同接口浏览器桥** 抹平传输层差异；**NestJS 按域拆服务** 复用现有 TS 业务逻辑；**PG/Redis/S3** 替换单机 SQLite/文件系统；**K8s 沙箱 Pod 编排（gVisor/Kata）+ 预热容量 + 每租户 PVC** 承接原来跑在用户机器上的 OpenClaw 运行时。三条硬主线：一切带 `tenant_id`、沙箱假设被攻破、模型出网收敛到网关。
