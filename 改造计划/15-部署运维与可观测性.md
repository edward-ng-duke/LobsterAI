...
| 沙箱运行时 | 普通容器（降级，提速开发） | gVisor/Kata（与 prod 对齐） | gVisor 默认 / Kata 高合规与重兼容档（强制，见 `07` §8.1） |
| PostgreSQL | 单实例（Docker/Operator） | 主+副本，日备 | 主+只读副本，PITR，跨 AZ |
| Redis | 单实例 | 哨兵 | 集群/哨兵 |
| 对象存储 | 本地 MinIO | MinIO 集群 / 云 S3 | 云 S3 / MinIO 多副本 |
| 模型/ASR 供应商 | mock / 小额真 key | 真 key（限额） | 生产 key，配额+计费开启（`09`） |
...
| `pool-sandbox-kata` | 沙箱 Pod（企业/高合规档，或 gVisor 兼容矩阵不达标的重/兼容资源档） | `kata` | 内存开销大，独立扩缩；按 `resourceClass`/租户策略调度 |
...
| `lobster-api` | API 网关/BFF + Auth/Cowork/File/Model/MCP/Skill/Artifact/Config 等 NestJS 逻辑模块（V1-V3 合并部署） | `node:24-slim`（对齐 Node `>=24.15.0 <25`） | monorepo `apps/api` | `lobster-svc` |
| `lobster-runtime-orchestrator` | 运行时编排器（`07`），负责沙箱 Pod 生命周期、Config Sync 触发、gateway lease | 同上 + kube client | `apps/runtime-orchestrator` | `lobster-svc`（唯一持 K8s 写权限） |
...
| `lobster-runtime-orchestrator` 镜像内 `config-sync` entrypoint（非独立 `lobster-configsync` 镜像） | 渲染 `/state/openclaw.json` + PVC `/workspace/state` 托管文件（`07` §5） | 同 `lobster-runtime-orchestrator` 镜像 + 去 Electron 化后的 Config Sync 包 | `apps/runtime-orchestrator` + `libs/server/config-sync`；不创建独立 `apps/configsync` | 沙箱 Pod initContainer 或 orchestrator 子命令 |
| `lobster-egress-proxy` | 审计型出站代理（`07` §8.4） | Squid/Envoy 官方镜像 + 配置 | 配置化 | `lobster-sandbox` |

> **部署单元冻结**：PR-0~PR-4 前置与 V1-V3 的物理镜像以 `lobster-api`、`lobster-runtime-orchestrator`、`lobster-worker` 为准；`04` 的领域服务表是逻辑模块，不代表立即拆成多个 `apps/*`。若 V5/V6 或规模化后因 HPA、权限隔离、发布节奏、故障域或团队所有权需要拆分，可从 `lobster-api` 内拆出 `lobster-gateway`、`lobster-cowork`、`lobster-auth`、`lobster-modelgw` 等独立镜像，但必须以监控数据和拆分 RFC/ADR 为前置，并同步更新 `02/04/15/19`、Helm values 与 CI 矩阵。
>
> **后端 monorepo 分层构建**：所有 `lobster-*`（NestJS）共享一个 base 层（`node:24-slim` + 生产依赖 + 编译产物），各服务只叠自己的 entrypoint，显著减小体积与拉取时间（呼应 `02` §4.10「镜像分层构建，共享基础层」）。构建只 lift-as-is 复用真正无状态、无 Electron/SQLite 依赖的 TS 纯逻辑（如 `artifactParser`、模型格式转换、部分 MCP/skill helper）；`coworkStore`/`openclawConfigSync` 只能复用语义、常量和 golden fixtures，服务端实现必须重写/去 Electron 化移植（见附录 C D15）。
...
├── values-prod.yaml
└── templates/
    ├── api/                         # V1-V3: 合并 API deployment/service/hpa/pdb/netpol
    ├── runtime-orchestrator/        # 独立编排器，唯一持 K8s 写权限
    ├── sandbox/                     # 沙箱 Pod 模板、预热容量 controller 配置、RuntimeClass 引用、egress-proxy
    ├── worker/                      # BullMQ worker
    ├── split-services/              # V5/V6 或规模化后才启用的 gateway/cowork/auth/modelgw 拆分模板
...
sandbox:
  runtimeClassName: gvisor        # 高合规/企业或重兼容资源档覆盖为 kata
  namespace: sandbox-tenant-a     # 示例值；模板为每租户 sandbox-<tenantId>（见 §1.2）
...
flowchart LR
  ...
  LINT --> BE["build 后端镜像 (api/worker/orchestrator)"]
...
| 敏感密钥 | 密钥管理系统（Vault / 云 KMS）→ External Secrets Operator 同步为 K8s Secret | DB 密码、Redis 密码、对象存储 AK/SK、OIDC client secret、JWT 签名密钥、**模型/ASR 供应商上游 key** |
| 每会话运行期注入（含租户上下文） | 编排器动态生成 → K8s Secret → Pod env | `OPENCLAW_GATEWAY_TOKEN`（每 Pod 唯一）、`LOBSTER_MCP_BRIDGE_SECRET`、会话级模型代理短期凭证（`07` §5.4、`09`） |
...
5. **最小权限**：编排器 ServiceAccount 通过 ClusterRole + 每租户 `sandbox-*` RoleBinding 模板获得受管租户 namespace 内 Pod/PVC/Secret/NetworkPolicy/ResourceQuota 的最小写权限，不授予集群级通配写权（`07` §3.1）；沙箱 Pod 挂**无 SA token**（`07` AC-3），防 Pod 内调 `kubectl`。
...
|  | `ws_connections_active`（Pods）| averageValue **300** | `avg by (pod) (ws_connections_active{namespace="lobster-svc",app="lobster-api",component="gateway"})` | WS 长连是主压力源，纯 CPU 不足；V1-V3 仍在 `lobster-api` 内，只用 `component` 区分逻辑模块 |
| Cowork 逻辑模块 | `cowork_active_sessions`（Pods）| averageValue **200** | `avg by (pod) (cowork_active_sessions{namespace="lobster-svc",app="lobster-api",component="cowork"})` | 承接流式广播；若 V5/V6 拆出 `lobster-cowork`，需先按 §2.1 更新 label 与 Helm 矩阵 |
| 模型网关逻辑模块 | `model_inflight_requests`（Pods）| averageValue **50** | `avg by (pod) (model_inflight_requests{namespace="lobster-svc",app="lobster-api",component="modelgw"})` | 上游 SSE 转发 IO 密集；V1-V3 不创建独立 `lobster-modelgw` |
...
- **沙箱 Pod 本身无需 DR**：Pod 是可丢弃的；工作区持久性不由 Pod 保证，而由 §7.1 的 PVC / 对象存储 / PostgreSQL 备份与 RPO/RTO 承担。故障恢复=重建 Pod + `chat.send` 续接（RPC 真名见附录 C D6；`07` §9 / 附录 C D9）；active turn 按 D9 受限续接，允许至多丢当前 turn。
...
| 对话首字延迟 | user 消息 → 首个 `cowork:stream:message` / `cowork:stream:messageUpdate`（按附录 C §3.2 的真实前端桥事件全集）p95 | 以 `17` V4 p95 表为准，区分预热容量命中与冷启 |
| 沙箱冷启 | acquire→ready p95 | 分预热容量命中与纯 cold 两条 SLI；具体 p95 阈值以 `17` V4 权威表为准，本表不再写死具体秒数（对齐 `07` AC-6） |
| 模型网关 | 代理成功率（排除上游 4xx） | 99.5% |
| 流式完整性 | turn 无丢事件完成率 | 99.9% |
| 数据持久 | 已提交会话/消息正常路径 0 丢失；DR 恢复按 PG PITR RPO≤5min，不承诺 active turn 无损（见附录 C D9） | RPO ≤ 5min |
...
桌面端「本地应用、无运营承诺」在 SaaS 下必须补齐面向租户的运营三件套；本节承接要点，**不得把完整支持流程/SLA 商务条款推迟到 GA 之后**。版本归属按 `14` §13：V5 付费试点前冻结最小支持分级、状态页组件模型、SLA 违约 credit 口径与计费对账接口；V6 GA 前由 SRE/PM + 客服负责人 + 法务/财务签收并通过 DP-17 演练。更复杂的客服系统选型或商务套餐细分可作为后续优化，但不能替代 V6 的状态页演练、P1 工单到 on-call 流程和 SLA 违约计量样例。
...
| **状态页（status page）** | 公开组件级健康 + 事件时间线 | 组件维度：Web/API、对话/沙箱、模型/ASR 网关、文件、定时任务；由 SLO 探针 + 合成监控驱动自动置红/黄；维护窗口预告；历史事件与 postmortem 摘要留档 |
...
- **载体**：集中式 flag 存储（Redis 中心 + 后端服务/编排器订阅热更新，或成熟 flag 平台），与镜像发布**解耦**；变更即广播、各服务 ≤60s 内生效，**不重建 Pod、不断连**。成熟 flag 平台选型可以后续替换，但**最小 kill-switch 控制面不可后置**：按 `14` §13，SRE/SEC + 平台负责人须在 V5 前提供可用控制面，V6 完成 ≤60s 停用演练、RBAC 双人复核与审计签收。
- **粒度**：全局 / 按租户 / 按套餐 / 按 provider / 按能力位；kill-switch 为「就地禁用」的粗粒度开关，特性开关为灰度/放量的细粒度控制。
- **高风险子系统（kill-switch 覆盖面）**：模型出网/某 provider、ASR 上游代理、MCP stdio 执行、技能供应链安装、公开分享/预览、BYOK、媒体生成、egress 放行项。命中 kill-switch 时返回明确降级错误（对齐统一错误信封，附录 C §3），而非静默失败。
...
| DP-5 | 密钥零明文 | 仓库/镜像无密钥（TruffleHog 通过）；上游模型 key 仅模型网关持有，ASR provider key 仅后端 ASR 代理持有，**沙箱 Pod / 浏览器不含真实 key** |
...
| DP-16 | kill-switch 生效 | 任一高风险子系统（模型出网/ASR/MCP/供应链/分享/BYOK 等）可在 **≤60s** 全网停用，不重建 Pod/不断连；操作有 RBAC + 审计（§8.5）|
...
2. **平台组件**：Ingress + cert-manager + Prometheus/Loki/Tempo/Grafana + gVisor/Kata RuntimeClass + Cluster Autoscaler + External Secrets + **PgBouncer（transaction 模式，一等组件）** + KEDA。
3. **镜像流水线**：改造 CI，删 electron 打包 job，新增 `lobster-api` / `lobster-worker` / `lobster-runtime-orchestrator` 镜像构建 + OpenClaw 运行时镜像（§2.3）+ 扫描 + cosign；细分多服务镜像仅在 V5/V6 或规模化拆分后加入矩阵。
...
- 模型/ASR 网关上游 key 托管、token/stream ticket 计费与用量指标对账：`09-模型代理与计费.md`
