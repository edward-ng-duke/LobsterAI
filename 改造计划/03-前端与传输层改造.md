...
> 本文档面向前端与全栈工程师，讲解如何把 LobsterAI 现有的 Electron 桌面渲染层，改造为运行在标准浏览器里的多租户 SaaS Web 前端。核心命题只有一个：**桥必须 1:1 实现整个 `window.electron` 全局表面**（同名、同签名、同返回类型的“Web 桥”，REST + WebSocket），而**不是“替换 services 层”**——因为渲染层组件直连 `window.electron` 的点数已过半于经 `services/*` 的点数（**245 > 207**，见附录 C A4/B10），只有补齐整张全局表面，才能在几乎不改 React 业务代码的前提下完成 Web 化。
>
...
**改造的第一性原理**：现有 **481 处**渲染层调用、**72 个文件**（见附录 C B10；口径以 `src/shared/*/constants.ts` 常量穷举为准，禁止用字面量 grep 计数）全部经由 `window.electron.*` 这一个 `contextBridge` 暴露对象访问（`src/main/preload.ts` 的 `contextBridge.exposeInMainWorld`）。其中**组件直连 245 处 > 经 `services/*` 的 207 处，过半绕过 services**（见附录 C A4）——因此桥必须 **1:1 补齐整个 `window.electron` 全局表面**，而非只替换 service 底层。只要该实现**同名、同签名、同返回类型**，业务组件就无需感知底层是 IPC 还是 HTTP。
...
对应 `preload.ts` 的 cowork 段。请求方法照搬；**10 个** `onStream*`（含原文档漏列的 `onStreamGoal`，即 `CoworkIpcChannel.StreamGoal`，见附录 C B13）/ `onSessionsChanged` 全部映射为 `ws.subscribe`。事件与通道全集必须从 `src/shared/*/constants.ts` 的 `as const` 对象穷举（附录 C D1），下列代码为示意而非事实源：
...
export function buildCoworkModule(http: HttpClient, ws: WsClient) {
  return {
    ...
    onStreamGoal: (cb: (d: { sessionId: string; goal: unknown }) => void) =>
      ws.subscribe('cowork:stream:goal', cb),
...
不要在 webBridge 里重新手写类型。把 `preload.ts` 暴露的**整张表面**（**全部方法**，而非仅 service 消费的子集——组件直连过半，见附录 C A4）抽为一个 `interface ElectronBridge { ... }`，preload 与 webBridge 都 `implements` 它，任一处漂移即编译期报错。该桥表面类型应**由 `libs/shared/contracts`（OpenAPI 3.1 + AsyncAPI 2.6）生成/校验**（附录 C D1），而非与后端契约各自手写。（抽接口可作收口阶段的加固，不阻塞主线。）
...
|---|---|---|---|
| 会话流式 | `cowork:stream:{message,messageUpdate,sessionStatus,contextUsage,contextMaintenance,permission,permissionDismiss,complete,error,goal}`（**10 类**，含原漏的 `goal`，见附录 C B13） | 主题名沿用原通道名 | 按 `sessionId` 过滤 |
...
> 正式 wire protocol 以 `04-后端服务与API设计.md` §5 为准，判别字段统一为 `type`。本节的 `channel`/`scope` 是浏览器桥订阅表的维度，后端可据此翻译为会话级 `subscribe {sessionId, sinceSeq?}` 或用户级自动订阅。

```jsonc
// 客户端 → 服务端（认证成功后）
{ "type": "subscribe",   "channel": "cowork:stream:message", "scope": { "sessionId": "s_123" }, "sinceSeq": 120 }
{ "type": "unsubscribe", "channel": "cowork:stream:message", "scope": { "sessionId": "s_123" } }

// 服务端 → 客户端（推送）
{ "type": "event", "channel": "cowork:stream:messageUpdate", "seq": 121,
  "data": { "sessionId": "s_123", "messageId": "m_9", "content": "…", "metadata": {} } }
{ "type": "event", "channel": "api:stream:r_abc:data", "seq": 7, "data": { "chunk": "…" } }
```
...
subscribe(topic: string, cb: (payload: any) => void): () => void {
  ...
  if (set.size === 0) this.sendControl({ type: 'subscribe', channel: topic });  // 首个订阅者才上报
  set.add(cb);
  this.handlers.set(topic, set);
  return () => {
    set.delete(cb);
    if (set.size === 0) this.sendControl({ type: 'unsubscribe', channel: topic });
  };
}
onFrame(frame: WsFrame) {
  if (frame.type === 'event') this.handlers.get(frame.channel)?.forEach((cb) => cb(frame.data));
}
```
...
- **WebSocket**：不直接把 access token 放 URL query，也不把长效 bearer token 作为首帧正式方案。正式方案为**一次性短期 WS ticket**：
  ...
  2. ticket 绑定 `tenantId/userId/sessionScopes/subscriptions/expiresAt/nonce`，TTL 建议 30-60 秒，只能消费一次。
...
- ticket 过期/失效时服务端关闭连接或返回 `{ type:'autherror', code:'TICKET_EXPIRED' }`。客户端先刷新 access token（如需要），重新申请 ticket、重连并重放订阅表（§6.2）。
...
- **订阅表重放**：`wsClient` 内部持有 `handlers`（topic→回调集合），重连前用订阅表申请新 ticket，认证后自动把所有活跃 topic 重新 `subscribe`，业务层无感。
...
- `saveInlineFile({ dataBase64, fileName, mimeType, cwd }) -> { success, path }`（现状 **25MB 上限**）：Web 下后端把内容落对象存储 / 租户工作区后，桥须把返回**适配回 `{ success, path }`**（`path` 改为工作区逻辑路径），以免打断 `ArtifactPanel`。
- `readFileAsDataUrl(...) -> { success, dataUrl }`：Web 下后端读工作区文件返回内容，桥把响应**适配回 `{ success, dataUrl }`**，以免打断 `CodeRenderer` / `ImageRenderer`。
- 其余 Electron-only 通道（`window.*` / `shell.*` / `dialog.*` / `clipboard.*` / `log.*`）在表面上**显式降级**：无 Web 对应者以 no-op 或抛“桌面专属”错误呈现，有对应者按上表给 Web 实现——但**方法必须在表面存在**，绝不能让直连组件访问到 `undefined`。
...
- 前端每个 HTTP 请求带 `Accept-Language: {当前语言}`；WS 的 ticket auth 帧（§6.1）可附带 `lang` 字段，供后端出通知文案时选择。
...
| 流式映射 | 全部 `cowork:stream:*`（**10 类**，含 `goal`，见附录 C B13）、`api:stream:${requestId}:*`（4 类）、各 `*:changed`（skills/mcp/auth/scheduledTask/artifact/sessions）均可经单条 WS 订阅收到 |
