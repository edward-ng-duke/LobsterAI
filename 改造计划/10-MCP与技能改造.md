...
`SkillManager`（`src/main/skillManager.ts`，约 3248 行，核对 2026-07-08）职责：
...
#### 4.1.3 MCP 工具授权与 AskUser 回调接线（历史评审编号 P0-2，非阶段门）
...
两者统一走**单一端点** `POST /api/v1/sessions/:id/permissions/:requestId/respond`（契约见附录 C §3.1；`requestId` 也保留在 body 中用于桥同形与幂等校验）：
...
校验时机与规则：
1. **创建/更新 MCP 时**（`POST /api/v1/mcp/servers`、`PATCH /api/v1/mcp/servers/:id`）先过策略（针对**用户声明的** command/args）：
...
-- 写：仅能改本租户行（平台基线不可被租户改写，只由 PR-3 seed / 受控维护 Job 审计更新）
...
|---|---|---|
| `mcp:list` | `GET /api/v1/mcp/servers` | 租户内列表 |
| `mcp:create` | `POST /api/v1/mcp/servers` | 先过 `mcpCommandPolicy` |
| `mcp:update` | `PATCH /api/v1/mcp/servers/:id` | 同上 |
| `mcp:delete` | `DELETE /api/v1/mcp/servers/:id` | 级联删 resolution |
| `mcp:setEnabled` | `PATCH /api/v1/mcp/servers/:id` | body `{enabled}`；不另开 `/enabled` 子路由 |
| `mcp:retryLaunchResolution` | `POST /api/v1/mcp/servers/:id/launch-resolution/retry` | 触发沙箱内重解析；禁止旧 `/resolve` |
| `mcp:fetchMarketplace` | `GET /api/v1/mcp/marketplace` | MCP 市场 |
...
sequenceDiagram
  ...
  FE->>API: POST /api/v1/skills/install {source|upload}
...
CREATE POLICY ssr_read ON skill_security_reports FOR SELECT
  USING (tenant_id IS NULL OR tenant_id = current_setting('app.tenant_id')::uuid);
CREATE POLICY ssr_tenant_insert ON skill_security_reports FOR INSERT
  WITH CHECK (tenant_id = current_setting('app.tenant_id')::uuid);
CREATE POLICY ssr_tenant_update ON skill_security_reports FOR UPDATE
  USING      (tenant_id = current_setting('app.tenant_id')::uuid)
  WITH CHECK (tenant_id = current_setting('app.tenant_id')::uuid);
-- 写入不得让常规 ScanWorker 使用 BYPASSRLS。租户扫描必须显式 SET LOCAL app.tenant_id 后写本租户行；
-- tenant_id=NULL 的平台镜像报告只由 PR-3 supply-chain seed / 受控维护 Job 写入并审计，若后续要在线平台扫描，
-- 需先补独立 RFC/策略，不得把 BYPASSRLS 作为默认逃生口。
```
...
- 新接口 `GET /api/v1/skills/routing-prompt`，按**当前租户已启用技能**（`tenant_skills.enabled=true` 且 `install_status=ready`）生成。
- 生成结果作为 **Config Sync 输入**随会话下发，由 `07` 定义的 `apps/runtime-orchestrator` config-sync entrypoint 渲染进 `AGENTS.md`/系统提示；Skills/MCP 服务不得直接写 Pod `/state` 或 `/workspace/state`。
...
| 现 IPC | 新接口 | 说明 |
|---|---|---|
| `skills:list` | `GET /api/v1/skills` | 平台镜像 + 租户覆盖合并 |
| `skills:setEnabled` | `PATCH /api/v1/skills/:id` | 仅改租户覆盖，body `{enabled}`；与 `mcp:setEnabled` 同口径，不另开 `/enabled` 子路由 |
| `skills:delete` | `DELETE /api/v1/skills/:id` | 私有技能删 S3+DB；平台技能仅本租户禁用 |
| `skills:download` | `POST /api/v1/skills/install` | source=url/upload；返回 202 + pending_scan；禁止回退到 `/skills:install` 或无版本前缀的 `/skills/install` |
| `skills:upgrade` | `POST /api/v1/skills/:id/upgrade` | 换 version，重扫 |
| `skills:confirmInstall` | `POST /api/v1/skills/:id/confirm` | body `{action}` |
| `skills:getRoot` | （移除；桥本地返回 `UnsupportedFeatureResult`） | Web 下无本地技能根目录概念，GA 主线不建普通 REST、不进入 route registry；如需暴露沙箱内约定路径，仅能作为诊断能力另走后续 RFC |
| `skills:autoRoutingPrompt` | `GET /api/v1/skills/routing-prompt` | 见 §5.6 |
| `skills:getConfig`/`setConfig` | `GET/PUT /api/v1/skills/:id/config` | 密钥转 secref |
| `skills:testEmailConnectivity` | `POST /api/v1/skills/:id/test` | 在沙箱内探测 |
| `skills:fetchMarketplace` | `GET /api/v1/skills/marketplace` | 自建 marketplace 目录；可用旧 skill-store 数据做一次性导入种子，但运行期不得代理 youdao/overmind |
| `skills:detectFromOpenClaw`/`syncFromOpenClaw`/`refreshPluginSkillIds` | `POST /api/v1/skills/sync` | 沙箱内 openclaw 扩展同步收敛为一个后端触发；禁止回退到冒号式 `/skills:sync` 或无版本前缀 `/skills/sync` |
...
sequenceDiagram
  ...
  FE->>API: POST /api/v1/kits/install {kitId}
...
| 现 IPC | 新接口 |
|---|---|
| `kits:fetchStore` | `GET /api/v1/kits/marketplace` |
| `kits:install` | `POST /api/v1/kits/install` |
| `kits:uninstall` | `DELETE /api/v1/kits/:id`（级联清理其独占的 skills/mcp） |
| `kits:listInstalled` | `GET /api/v1/kits/installed` |

卸载语义：只清理"该 kit 引入且未被其他 kit/手动引用"的 skills 与 mcp servers（引用计数）。connectors（IM 相关）在 GA 主线只记录不激活——IM 渠道为后续（见 `13-功能取舍与降级清单.md`）。

商店数据迁移：`getKitStoreUrl()`/`getSkillStoreUrl()`（`endpoints.ts`）当前指向 youdao overmind。按"全部自建"决策，后端自建 `GET /api/v1/kits/marketplace`、`GET /api/v1/skills/marketplace` 作为技能/包目录服务；旧 overmind / skill-store 只能作为**一次性目录导入或离线镜像种子**，不得在目标 SaaS 运行期由后端代理、透传或实时依赖。导入后的 marketplace 条目必须进入 `supply-chain-inventory.schema.json`，带来源、版本、checksum / integrity、扫描状态和下架策略。
...
### 7.1 供应链资产清单（V1 前必须入库）

仅写“安装后扫描”不够。V1 runtime 镜像 PoC 之前，必须先把当前仓库已经会进入 OpenClaw/Skills/MCP 执行面的资产清成机器可读清单，否则镜像可复现、离线生产、CVE 重扫和插件升级都没有事实源。

**资产来源（核对 2026-07-08）。**

| 来源 | 当前口径 | 清单责任 |
|---|---|---|
| 内置技能目录 | 由 inventory 脚本扫描当前仓库中参与 runtime 打包的 `SKILL.md` 实际路径（当前数量以 CI 扫描为准），不得硬编码大小写不明的技能根目录 glob | 生成 `kind='skill'` 条目，记录 `skillId/version/sourcePath` 与真实 `sourcePath` |
| `package.json.openclaw.plugins` | OpenClaw 第三方插件列表和版本 | 生成 `kind='openclaw-plugin'` 条目，记录插件 id、版本、下载源、完整性 |
| Kits / marketplace 镜像 | 技能包和附带 MCP 配置 | 生成 `kind='kit'` 条目，记录 bundle、skill 列表、mcpServers |
| MCP 包解析结果 | `npx/uvx/python/raw` resolution | 生成 `kind='mcp-package'` 条目，记录 package、registry、lockfile、scan id |

**建议落点。**

- `libs/shared/contracts/assets/supply-chain-inventory.schema.json`：清单 JSON Schema，进入 `contracts:check`。
- `docs/supply-chain/skills-and-plugins.manifest.json`：当前仓库 bundled skills、OpenClaw plugins、默认 Kits 的清单快照。
- `scripts/supply-chain/check-inventory.cjs`：扫描配置化 bundled skill roots + `**/SKILL.md` allowlist、`package.json.openclaw.plugins`、Kit manifest、lockfile，和 manifest 做差异校验；扫描结果必须输出真实路径，作为 manifest 的 `sourcePath`。
- `npm run supply-chain:check`：CI 门禁；凡 bundled skill roots、`package.json` 的 `openclaw` 段、OpenClaw plugin lock、Kit/MCP 清单变更而 manifest 未更新，直接失败。

**每条资产至少包含字段。**

```jsonc
{
  "id": "openclaw-plugin:example",
  "kind": "skill | kit | openclaw-plugin | mcp-package",
  "sourcePath": "<actual-bundled-skill-root>/example/SKILL.md",
  "packageManager": "npm | uv | python | none",
  "runtimeDeps": ["node>=24", "git"],
  "registry": "internal-npm-mirror",
  "lockfile": "sha256:...",
  "integrity": "sha512-...",
  "nativeModules": false,
  "requiresNetwork": "install-only | runtime-egress | none",
  "requiresFilesystem": "workspace-read | workspace-write | none",
  "license": "MIT",
  "scanPolicy": "required | platform-reviewed | blocked",
  "runtimeImageLayer": "openclaw-runtime | skill-cache | tenant-cache",
  "owner": "platform"
}
```

**V1 阻断规则。** `lobster-openclaw-runtime` 镜像 PoC 不得在 manifest 为空、缺项或 `npm run supply-chain:check` 未通过时进入 V1 评审。manifest 可以先覆盖“当前会被镜像打包/会被默认启用”的最小集合，但不能只在文档中口头说明来源。

### 7.2 安装与缓存策略（扫描之外的硬门槛）
...
| overmind 商店依赖 | 与"全部自建"冲突 | 自建 marketplace 服务；旧目录只允许一次性导入/离线镜像，运行期不得代理 youdao/overmind | `09`/`02` |
...
- **安全扫描**：现有 js-x-ray 规则、prompt 审计规则和 fixture 可作为服务端扫描的基线资产；服务端外壳、租户上下文、对象存储输入、worker 执行、审计与阻断门控必须重写/补齐，不能把当前本地扫描流程整体搬到控制面。
