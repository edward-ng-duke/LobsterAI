...
| 命令安全（stdio 白名单） | 当前**无**显式白名单（自由 command/args） | 新增 `commandSafety` 白名单/策略，成为多租户安全前置 | 高（新增） |

关键结论：
1. **`sse`/`http` MCP 几乎零成本可移植**——远程 HTTP 调用与桌面无关，只需把密钥（headers/token）迁到租户级密钥库，并套上 SSRF/出网白名单。
2. **`stdio` MCP 是最大风险面**：现状允许任意 `command`+`args`（`src/main/mcp/mcpStore.ts:11-16`），运行的是任意 npm 包的可执行文件。SaaS 化后**必须**：(a) 只在租户沙箱内执行、(b) 引入命令白名单 `commandSafety`、(c) 强制安全扫描门控。
...
非 npx 的 stdio 走 raw 路径（`resolveStdioCommand`，`mcpRuntime.ts:206-215`），**直接执行用户给定的任意 command**。
...
安全扫描类型（`skillSecurityTypes.ts`）：8 个维度 `file_access|dangerous_command|network|process|screen_input|payment|prompt_injection|web_content`，4 级 severity，风险分（`skillSecurityScanner.ts:53-58`：info=0/warning=5/danger=20/critical=50）。用户可选 `install|installDisabled|cancel`（`:35`）——**当前是建议式，不强制**。
...
| 模型 | 说明 | 取舍 |
|---|---|---|
| **A. 沙箱内即时解析（推荐 v1）** | 沙箱 Pod 启动时，在 Pod 内跑等价 `McpLaunchResolverManager` 逻辑（npm install 到 PVC/临时层），gateway spawn | 隔离彻底；每 Pod 首启有安装延迟（可缓存到租户 PVC） |
| **B. 托管 MCP 运行器（后续）** | 平台侧预构建常用 MCP 的镜像/包缓存，沙箱挂载只读层 | 冷启动快、供应链可控；需维护镜像目录 |

v1 采用 A，为 B 预留 `mcp_launch_resolutions.resolver_kind` 扩展位（现有 `npx|uvx|python|raw`，见 `mcpLaunchResolution.ts:5-10`）。
...
### 4.2 命令安全：`commandSafety` 白名单（新增）

**现状无任何命令白名单**——`command`/`args` 完全由用户提供并直接执行（`mcpStore.ts:11-13` + raw stdio 路径 `mcpRuntime.ts:206-215`）。单机可接受，多租户**必须**收口。新增策略层 `commandSafety`：

策略模型（存 `mcp_command_policy`，平台级 + 租户可覆盖收紧）：

```ts
// 建议置于 src/shared/mcp/commandSafety.ts（跨进程共享）
...
export interface McpCommandPolicy {
  ...
  allowedCommands: string[];   // 例：['npx','uvx']（基名匹配）
  allowedPackages?: string[];  // npm 包名白名单（可选，收紧）
  blockedArgPatterns: string[];// 例：['--allow-run','-e','--eval']
  requireScan: boolean;        // 强制安全扫描通过
}
```

校验时机与规则：
1. **创建/更新 MCP 时**（`POST/PATCH /mcp/servers`）先过策略：
   - stdio 且 `mode=managed_npx_only`：只接受能被 `isNpxMcpServer` 识别的命令（`mcpLaunchResolution.ts:60-68`），拒绝任意二进制。
   - `mode=allowlist`：`basename(command)` ∈ `allowedCommands`；`args` 不含 `blockedArgPatterns`。
   - 命中 `allowedPackages` 时进一步限制可安装包。
2. **解析包名后二次校验**（`parseNpxArgs` 之后）：`packageName` ∈ `allowedPackages`（若启用）。
3. 违规 → 返回 `422`，写审计日志（见 `14`），不落地。
4. `requireScan=true` 时：MCP 包安装完成后触发一次静态扫描（复用 skillSecurity 引擎扩展到 node_modules 抽样），critical 命中则标记 `blocked`，沙箱不加载。
...
| `mcp:create` | `POST /mcp/servers` | 先过 `commandSafety` |
...
class CreateMcpServerDto {
  ...
  @IsString() command?: string;          // 交给 commandSafety 复核
...
sequenceDiagram
  ...
  alt riskLevel critical / requireScan 未过
...
门控策略（区别于现状"建议式"）：
- `riskLevel ∈ {safe,low}` → 自动 `ready`。
- `riskLevel ∈ {medium,high}` → `pending_scan`，前端展示报告，用户显式确认（`skills:confirmInstall` 等价接口）后置 `ready`，否则 `blocked`。
- `riskLevel = critical` → 默认 `blocked`；仅企业租户管理员可强制放行并记审计。
...
sequenceDiagram
  ...
  API->>PG: 批量 mcp_servers（过 commandSafety）
  API->>Q: enqueue 批量扫描（skills + mcp 包）
  API-->>FE: 202 {kitId, skills:[...], status}
  Note over PG: connectors → im/connector 配置（v1 若涉及 IM 走后续，见 13）
```
...
CREATE TABLE tenant_kits (
  ...
  connectors jsonb,               -- v1 记录不激活（IM 后续）
  installed_at timestamptz NOT NULL DEFAULT now(),
...
卸载语义：只清理"该 kit 引入且未被其他 kit/手动引用"的 skills 与 mcp servers（引用计数）。connectors（IM 相关）在 v1 只记录不激活——IM 渠道为后续（见 `13-功能取舍与降级清单.md`）。
...
| 任意命令注入 | 现状允许任意 command/args | `commandSafety` 白名单（§4.2），默认 `managed_npx_only` |
| 恶意 skill 脚本 | prompt 注入 / 数据外泄 / 危险命令 | 强制服务端扫描门控（§5.3），critical 默认 blocked |
...
| # | 验收项 | 判定 |
|---|---|---|
| M1 | sse/http MCP 密钥不明文入库 | DB 查 `header_secret_refs` 为 secref，明文只在沙箱内存 |
| M2 | stdio MCP 仅在沙箱内执行 | 控制面进程无任何 `spawn(command)`；审计日志确认 |
| M3 | `commandSafety` 默认拒任意二进制 | 提交 `command=/bin/sh` 返回 422 且不落地 |
| M4 | 租户隔离 | 租户 A 无法 list/get/update 租户 B 的 MCP（RLS + API 双重） |
| M5 | launch resolution 目标平台化 | 指纹不含控制面 arch；含 tenant_id；换租户重解析 |
| M6 | SSRF 防护 | sse/http URL 指向 169.254.169.254 被拒 |
| M7 | AskUser 回调 | 沙箱内 MCP 触发 AskUserQuestion → 前端经 WS 收到并可应答 |
...
| S2 | critical 默认 blocked | 注入含 data-exfiltration 的 skill，普通租户无法启用 |
...
| K3 | mcp 过 commandSafety | kit 内 mcpServers 同样走 §4.2 校验 |
| K4 | connectors v1 降级 | connectors 仅记录不激活，UI 明示"IM 渠道后续支持" |
...
| 现存 stdio 服务器无白名单历史数据 | 迁移期存量任意命令 | 迁移时全部标 `pending`，首次运行前强制过 `commandSafety` + 扫描 | `06` |
...
- **stdio MCP**：从"主进程本地任意 spawn"改为"沙箱内受管执行 + `commandSafety` 白名单 + 供应链扫描"，是本域最重的一块。
