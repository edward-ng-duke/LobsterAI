...
- 结论：**不重写前端**。浏览器桥须 **1:1 实现整个 `window.electron` 全局表面**（而非只替换 services 层，因过半组件直连桥）：`invoke(channel, ...)` → REST(HTTP)；`on('...:stream:*')` → WebSocket 订阅。前端 UI/Redux/组件几乎零改动。
...
| 独立 `HOME` / 单卷状态 / SQLite WAL / 优雅停机 | `07`、`08`、`15` | 映射为 Pod state 卷、PVC 工作区和 drain/落盘流程 |
...
| 版本 | 名称 | 核心交付物 | 验收信号 |
|---|---|---|---|
| **第一版 V1** | 可行性验证版 | 真实 OpenClaw gVisor Pod 跑通一条 turn；RWX/PVC+S3 存储 PoC；Web 桥契约冻结；Config Sync golden；模型出网封锁 PoC | V1 go/no-go 评审通过；沙箱、存储、桥、配置、egress 均有实测证据 |
...
| **第三版 V3** | 多租户 Beta 版 | Tenant/RBAC/RLS、工作区文件 API、Artifacts 多租户存储、服务端 BullMQ 定时任务、Beta 审计与删除导出 | 邀请制外部租户可试用；API/WS/S3/Redis/PVC/任务均通过跨租户红队测试 |
...
| **第五版 V5** | 商业化与生态版 | 模型网关计费、短期 model token、MCP/Skills 供应链治理、公开分享 abuse 治理、桌面导入工具 | 付费试点可用；模型调用不可绕过网关；stdio MCP 不在宿主执行；公开分享有治理流程 |
| **第六版 V6** | GA 上线运营版 | 全链路安全压测、混沌演练、可观测告警、发布回滚、on-call、SLO、合规与数据生命周期 | P0/P1 清零；DR/Chaos/回滚/on-call 演练通过；满足 GA 准入 |

```mermaid
timeline
  title 六版推进（阶段可部分并行）
  V1 可行性验证 : 真实OpenClaw Pod turn : 存储PoC : 桥契约 : egress封锁
  ...
  V5 商业化生态 : 模型网关计费 : MCP/Skills供应链 : 公开分享治理 : 桌面导入
  V6 GA运营 : 安全压测 : 混沌演练 : 告警回滚 : SLO合规
```
...
| **R3** | **数据迁移正确性**：SQLite→Postgres 多租户，历史列名与无租户模型残留 | 高（数据错乱/串户） | 全表补 `tenant_id`、迁移脚本 + 双读校验、**强制 RLS(FORCE)** 兜底（见附录 C D2）、灰度回滚。见 `06`、`05` |
...
| **R5** | **自建云能力交付超期**：账号/计费/分享/技能商店全部重建，范围大 | 中（进度风险） | 按六版切分，先证伪高风险假设，再交付 Beta、运行时硬化与商业化；非必需（完整技能市场 UI 等）延后；复用现有 TS 逻辑降低重写量。见 `09`、`17`、`18` |
...
>
> **口径说明（评审项 P1-12）**：本节 30–45 人月为**含缓冲系数的自顶向下区间估**，与 `17` 的 WBS **自底向上合计（≈25 人月）**并不自洽。**以 `17` 的 WBS 为权威口径**，本区间仅用于决策沟通与容量冗余判断，不作承诺。
...
关键不确定性集中在 **V1（可行性验证）** 和 **V4（运行时强化）**：若 V1 无法证明沙箱、存储、桥契约和模型出网封锁可行，应暂停或调整路线；若团队 K8s/gVisor/RWX 存储经验不足，V4 周期与人月应向区间上限取值。

---

## 8. 文档导航

本计划共 22 份文档（19 篇正文 + 3 篇附录）。建议按角色选读，交叉引用见各处「见 XX 文档」。**关键跨文档口径（决策基线 D1–D16、源码订正、接口契约）以附录 C 为准。**
...
- **V1-V6 GA 主线范围**：认证多租户 + 模型代理与计费 + 对话/Agent/Artifacts/Skills/MCP + 文件工作区读写 + 定时任务；**不做** IM（GA 后续）、computer-use、VM/后台浏览器。
